/**
 * Stickiness coaches (widget + leave-reminder tips).
 * IIFE-scoped: classic scripts share one global lexical env, so top-level
 * `const ENGAGEMENT_KEY` throws SyntaxError if this file is evaluated twice
 * (CAPACITOR-13).
 */
(function (global) {
  if (global.nextTrainStickinessCoaches) {
    return;
  }
  ...
  function anyJourneyHasRemindersOn() {
    const journeys = global.nextTrainApp?.getConfiguredJourneys?.() ?? [];
    return journeys.some((journey) => journey?.remindMe === true);
  }

  function hasSkippedTemplateWizard() {
    return Boolean(global.nextTrainApp?.hasSkippedTemplateWizard?.());
  }
  ...
  async function evaluateStickinessCoaches() {
    ...
    if (!global.nextTrainApp?.hasJourneyKind?.()) {
      return;
    }
    ...
    if (shouldShowWidgetCoach(engagement, widgetState)) {
      const shown = await global.nextTrainWidget?.showWidgetCoach?.();
      if (shown) {
        return;
      }
    }
    ...
    if (shouldShowReminderCoach(engagement, latestWidgetState, reminderState, false, true)) {
      global.nextTrainLeaveReminders?.showLeaveReminderCoach?.();
    }
  }

  function scheduleStickinessCoachEvaluation() {
    global.setTimeout(evaluateStickinessCoaches, EVALUATION_DELAY_MS);
  }
  ...
  global.nextTrainStickinessCoaches = {
    readEngagement,
    readCoachState,
    recordAppOpen,
    recordFirstConfiguredJourney,
