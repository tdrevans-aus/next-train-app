...
| **FB-25** | **Code review Phase 2 — split `app.js`** | One PR per module: combobox → journey-model → train-navigation → nearby-mode → template-wizard → journey-detail. **Decision (Aug 2026):** Option A — plain script files + `window.nextTrain*` globals loaded before `app.js` (not esbuild bundle yet). **Progress:** 2.1 `station-combobox.js` ✅, 2.2 `journey-model.js` ✅, 2.3 `train-navigation.js` ✅; 2.4–2.6 blocked on nearby/wizard coupling — see `docs/codebase-inventory.md`. Branch `cursor/fb-25-split-app-js`. | **In progress** (Aug 2026) — 3/6 modules |
