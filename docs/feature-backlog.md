...
| **FB-53** | **West Midlands Metro: live both-directions check during service hours** | Adapter merges both TfWM platform ids per stop into one board (#232, 5 Sep 2026), proven only against a synthetic feed — Jim's live pull ran ~03:00 Birmingham time with no trams in the feed. Next step: a Mark run during Metro service hours, roughly 13:00 to midnight Perth time, to verify Jewellery Quarter or The Hawthorns shows departures both ways. If it passes, move the Metro board-eligibility verdict in `docs/uk-west-midlands-d1/oracle-clash-report.md` from `out-product` to `in`; no code change needed. | **To do** (added 5 Sep 2026) |
