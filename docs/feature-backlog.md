...
| **FB-48** | **QA: `reminders-permission-gate.mjs` in-sequence flake** | Fails inside `run-all --smoke` (right after `reminders-dialog.mjs`) but passes standalone. Suspect dev-server rate-limit contamination (`lib/api-rate-limit.js`, 60 req/min unless `CI=true`) — same class as the deferred-load bug fixed in PR #144. Repro: exhaust bucket with ~70 `/api/board` fetches, then run the script. Surfaced 30 Aug 2026 once the suite got past the reminders-dialog hang. | Backlog — **Aug 2026** |
| **FB-49** | **Rotterdam: `rotterdam-mark-probes.mjs` empty midday board** | Deterministic standalone failure: `Tussenwater: empty board at midday` (line 14/43). First seen Sat 29 → Sun 30 Aug 2026 — likely weekend-timetable gap in the Rotterdam pack (`docs/rotterdam-d1/`, `lib/providers/rotterdam.js`) or a date-dependent probe. Fix data or pin the probe to a weekday; verify on weekday + weekend. Mark-lane QA. | Backlog — **Aug 2026** |
