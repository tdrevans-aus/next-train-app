...
| **FB-56** | **Send the Nexus (Tyne and Wear Metro) live-data request after store launch** | Draft is ready at `docs/outreach-drafts/north-east-nexus.md` (recipients verified: customerrelations@nexus.org.uk To, mediarelations@nexus.org.uk Cc; plain-English, 5 Sep 2026). Tim's call (5 Sep 2026): do not send yet — wait until Next Train is live on the App Store and Play Store, then add a store link to the email so Nexus can see the product before replying. Until then North East ships National Rail only with the Metro `out-product` (#255, ledger §3). If Nexus says yes, the follow-up is a small Jim brief: swap `MetroFeedUnconfirmedError` for a real fetch; catalog and direction model are already in place. | Waiting on store launch |
| **FB-57** | **Surface live vs timetable on the board (NL first, then every GTFS-RT city)** | PR #261 (5 Sep 2026) gives the Rotterdam/Amsterdam adapters a `realtime: "live" or "timetable"` field on their internal board result, set from whether the shared OVapi TripUpdates cache supplied corrections or the request fell back to the static timetable. It is NOT yet threaded through `api/next-train.js` into the public response, and no UI hint exists anywhere in `public/` (Jim checked). Scope: (1) pass the field through the API for every city that has it (optional, absent = unknown); (2) one small board marker, e.g. "live" / "timetable" next to the fetched-at time, shown only when the field is present; (3) reuse for any other GTFS-RT city (Sydney, Perth etc.) where the adapter already knows. Product rule this serves: a rider must be able to tell a live time from a scheduled one (Tim, 5 Sep 2026). | Open |
| **FB-58** | **Göteborg: live boards via Västtrafik Planera Resa v4** | Göteborg is tester-live but schedule-only; Trafiklab still lists no `vt` TripUpdates (table dated 2026-09-05). Tim's rule (5 Sep 2026): a city with a usable live feed must use it or be delisted. Västtrafik has one: free instant registration at developer.vasttrafik.se, OAuth2 client-credentials, `GET https://ext-api.vasttrafik.se/pr/v4/stop-areas/{gid}/departures` with `plannedTime`/`estimatedTime`. Brief: `docs/jim-brief-goteborg-vasttrafik-live.md` (adds `lib/providers/vasttrafik.js`, Trafiklab static stays as fallback, sets the FB-57 `realtime` field). Needs Tim to register the app and put `VASTTRAFIK_CLIENT_ID`/`SECRET` on Vercel; rate limit and v4 licence to be read from the subscription page and recorded before flip. Decision 6 Sep 2026: use it, do not delist. | Open — after UK regions |
| **FB-59** | **Vienna adapter parked on branch `vienna-adapter` (PR #289, closed 6 Sep 2026)** | Complete, green planned adapter for Wiener Linien U1/U2/U3/U4/U6 (99 stations, static GTFS, schedule-only, Karlsplatz hub) built 6 Sep 2026 and deliberately closed unmerged because Austria is out of scope during the UK push. Nothing is lost: branch is on origin. To resume: reopen PR #289, rebase onto master (expect trivial conflicts in `lib/providers/registry.js` and `qa/run-all.mjs`), re-run `node qa/vienna-planned-gate.mjs`, merge. Then Mark. Tracker row Vienna, Stage 2 Packed, Waiting on points here. | Parked — until Austria is next |
| **FB-60** | **Station mode parameter through the API (dual-mode regions)** | Raised by `docs/jim-brief-uk-west-midlands-metro-shared-names.md` (6 Sep 2026): West Midlands' Five Ways, Jewellery Quarter and The Hawthorns each share a printed name between a National Rail entry and a West Midlands Metro entry in the same catalog, and neither `/api/directions` nor `/api/next-train` accepts a `mode` query parameter — so `resolveCatalogEntry()`'s rail-first default always wins and a rider can never reach the tram board at those three stops without a disambiguated catalog name. Fixed for West Midlands by renaming the three Metro entries with a `(Metro)` suffix (no API change) rather than threading a mode parameter, since the rename was sufficient there. Threading `mode` through `api/directions.js`, `api/next-train.js`, `lib/cities/live-city-api.js` and `public/app.js` is the correct long-term shape if a third dual-mode region needs it, since the rename trick doesn't scale past visually distinguishing an unbounded number of clashing names. Second known instance, currently moot: East Midlands' Nottingham Station (same NR/tram clash shape) — moot today because Nottingham Express Transit (NET) is out-product there. | Open |
