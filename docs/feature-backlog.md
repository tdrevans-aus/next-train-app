...
| **FB-54** | **Tamworth (TAM): service-hours direction probe** | Tamworth was added to the West Midlands catalog per the UK ledger stop-ownership decision (#238, 5 Sep 2026). Jim's live probe ran ~05:30 UK time, before Cross-City services start: 8 trips, destinations Crewe / London Euston / Edinburgh / Nottingham / Plymouth, operators LNR & WMR / CrossCountry / Avanti, and no "Birmingham New Street" chip — so no hub-anchoring entry was added, correctly. Re-probe `node scripts/probe-uk-board.mjs --crs=TAM` during UK daytime (07:00–19:00 Europe/London): if an operator split appears on "Birmingham New Street" (WMR vs LNR/Avanti), add a `direction-hubs.json` entry in the Kidderminster style; if every Birmingham-bound chip is a single operator, record that and close. Same shape as FB-53. |
