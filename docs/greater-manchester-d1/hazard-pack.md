...
| **Stockport** | National Rail station (SMN) vs Metrolink tram-only stops nearby | Report line 19: "Metrolink does not serve Stockport town centre rail station... Metrolink serves tram-only stops in Stockport (e.g., Stockport tram stop on Altrincham and Ashton lines)" — **not co-located**, not modelled as a doNotGroup pair (report explicitly frames this as separate modes/separate locations, not a walk-link relationship the way Piccadilly's pair is), just two catalog entries under different names that must not be conflated. |
...
| Stockport (NR) vs Stockport tram stops (Metrolink) | **Proposed, not built as a pair** | Report line 19: separate modes, not co-located, not even a walk-link relationship the report describes in interchange terms — just two catalog entries under similar names that must not be conflated. |
