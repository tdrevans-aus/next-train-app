...
**Merseyrail verdict — corrected 2 Sep 2026 at flip time:** Merseyrail is walk-up only, no reservation system, and passes both boarding-contract tests on paper. But `fetchMerseyrailStopBoard()` in `lib/providers/liverpool-city-region.js` is genuinely **not implemented** — it throws `MerseyrailFeedUnconfirmedError` unconditionally on every call, the same as East Midlands' NET tram and West Midlands' Metro. This D1 pack's original text called that shape "schedule-only," but no schedule-only board was ever built here (or in Greater Manchester's Metrolink or South Yorkshire's Supertram, which share the identical unconditional-throw pattern) — "schedule-only" was aspirational language, not shipped code. A service that can never actually appear on a board is `out-product`, not `in`, by the same test East Midlands and West Midlands already applied to their own unconfirmed-feed layers.
- **Merseyrail Northern Line and Wirral Line (all 69 stations)**: `out-product` — passes both walk-up tests, excluded because the real-time feed is unconfirmed (static GTFS is confirmed live via Transitland, but no public GTFS-RT endpoint exists anywhere). Board surfaces an explicit `MerseyrailFeedUnconfirmedError` rather than silently omitting the service or fabricating a schedule.
