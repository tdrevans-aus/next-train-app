...
**Merseyrail verdict — CORRECTED AGAIN 4 Sep 2026 (supersedes the 2 Sep correction below):** Merseyrail is a National Rail train operating company (a concession, like every other TOC), not a metro system without a public feed. Its stations carry CRS codes and its services are served by Darwin/OpenLDBWS exactly like Northern's or Avanti's. Probed live 4 Sep 2026 (`node scripts/probe-uk-board.mjs --crs=ELP,LVC,MRF,LIV`): Ellesmere Port, Liverpool Central, Moorfields and Liverpool Lime Street all returned trips with `operator: "Merseyrail"`, live platform and lateness data. `fetchMerseyrailStopBoard()` in `lib/providers/liverpool-city-region.js` now calls Darwin via `uk-darwin.js`'s `fetchStationBoard()` (pre-resolved entry/mode passthrough), the same shared path and same `MissingDarwinTokenError`-on-no-token behaviour as National Rail. **Verdict: `in`.** See `docs/jim-brief-liverpool-merseyrail-via-darwin.md` for the full evidence and decision record (Tim, 4 Sep 2026).
- **Merseyrail Northern Line and Wirral Line (68 stations, Lime Street's Merseyrail presence folded into the single mode-train Lime Street entry per Tim's option B)**: `in` — passes both walk-up tests, and now has a live real-time board via Darwin, same as every other TOC in this catalog. No operator filters are applied anywhere in this region (`docs/board-eligibility-rule.md` walk-up rule).

**HISTORICAL — Merseyrail verdict corrected 2 Sep 2026 at flip time (superseded 4 Sep 2026, see above):** Merseyrail is walk-up only, no reservation system, and passes both boarding-contract tests on paper. But `fetchMerseyrailStopBoard()` in `lib/providers/liverpool-city-region.js` was genuinely **not implemented** — it threw `MerseyrailFeedUnconfirmedError` unconditionally on every call, the same as East Midlands' NET tram and West Midlands' Metro. This D1 pack's original text called that shape "schedule-only," but no schedule-only board was ever built here (or in Greater Manchester's Metrolink or South Yorkshire's Supertram, which share the identical unconditional-throw pattern) — "schedule-only" was aspirational language, not shipped code. A service that can never actually appear on a board is `out-product`, not `in`, by the same test East Midlands and West Midlands already applied to their own unconfirmed-feed layers.
- **HISTORICAL, superseded 4 Sep 2026: Merseyrail Northern Line and Wirral Line (all 69 stations)**: `out-product` — passes both walk-up tests, excluded because the real-time feed was believed unconfirmed (static GTFS was confirmed live via Transitland, but no public GTFS-RT endpoint was found anywhere). That search used the wrong lens (GTFS-RT, not Darwin) — see the 4 Sep correction above.
...
| **Merseyrail Northern and Wirral Lines** | Liverpool Central, Moorfields, Ellesmere Port, and 66 other Merseyrail stations | No (walk-up commuter urban rail, no reservation system) | No | `in` | Passes both tests. Served live by Darwin (Merseyrail is a National Rail TOC with CRS codes); verified 4 Sep 2026 via `node scripts/probe-uk-board.mjs --crs=ELP,LVC,MRF,LIV`. Earlier `out-product` (2 Sep 2026) rested on the false belief that no feed existed — see the dated correction above. [Merseyrail](https://www.merseyrail.org/). |

**Board eligibility summary:** All walk-up National Rail services (Northern Trains, Avanti, TransPennine, East Midlands, TfW, West Midlands, CrossCountry) calling at Liverpool Lime Street and regional City Region stations pass both boarding-contract tests and are `in`. Merseyrail's 68 stations also pass both tests and are `in` — served live by Darwin (corrected 4 Sep 2026; the earlier `out-product` verdict is historical). Sleeper services (Caledonian Sleeper) fail test 1 (compulsory reservation) and are `out-reservation`. **All verdicts recorded; no silent omissions.**
