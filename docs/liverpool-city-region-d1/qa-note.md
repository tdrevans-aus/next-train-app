...
| **V1 scope vs oracle report** | ✓ CONFIRMED | National Rail + Merseyrail (schedule-only) — deliberate deviation from oracle's headline recommendation, justified and flagged in direction-model-memo.md as open item 1; board-eligibility verdicts lock this choice (Merseyrail `in` per oracle line 84) |
...
**Merseyrail Layer:** Fetching Liverpool Lime Street in metro mode correctly surfaces `MerseyrailFeedUnconfirmedError` rather than returning board data (real-time feed genuinely unconfirmed, static schedule available). Not a blocker; Tim and Luke both flagged this as permanent-by-design, shipped schedule-only after board eligibility verdict `in`.
