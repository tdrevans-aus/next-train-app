...
| 2026-09-22 | **A direction label is an API contract.** Renaming one (e.g. #439's Melbourne/Adelaide terminus-only relabel) requires a server-side alias for every retired label form, kept for at least one app release cycle, in addition to any client-side migration — an installed app or a cached web bundle keeps sending the old label until it's rebuilt/reinstalled, and the server must still resolve it to a real trip and echo the canonical label back so the client self-heals its saved value on next write. Client-only migration (`LEGACY_DIRECTION_ALIASES` in `public/journey-model.js` alone) caused a live "No upcoming trains" regression for every installed Melbourne/Adelaide route (docs/jim-brief-direction-label-aliases-server-side.md). The alias table for each affected city lives once, as data, in `lib/cities/<city>/direction-label-aliases.json` — generated by `scripts/generate-direction-label-aliases.mjs` from that city's own canonical line/terminus data — and is read by both the server (`lib/cities/direction-label-aliases.js`) and the client (the generated block in `public/journey-model.js`), never hand-kept twice.
