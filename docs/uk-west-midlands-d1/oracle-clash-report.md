...
- **West Midlands Metro Lines 1 & 2 (Birmingham–Wolverhampton, light rail):** `in` — Walk-up service (light rail, no reservation system), passes both boarding-contract tests. Credentials registered, `stopIds[]` platform merge landed (#232), and a live check during Metro service hours on 5 Sep 2026 confirmed both directions return real departures at a mid-line station (see 5 Sep 2026 live-check entry below). **Reason:** "TfWM GTFS-RT live feed confirmed returning both-direction departures at a mid-line Metro station during service hours; no remaining blocker."
...
| **West Midlands Metro Line 1 & 2 (light rail, Birmingham–Wolverhampton)** | 35 Current Service stops (Birmingham city centre to Wolverhampton) | No (walk-up only, light rail with no reservation system) | No | `in` | Live check 5 Sep 2026 during Metro service hours: both directions ("Edgbaston Village" southbound, "Wolverhampton St George's"/"Wolverhampton Station" northbound) returned real departures at Jewellery Quarter and The Hawthorns (mid-line, two-platform stops). |
...
**Board eligibility summary:** All walk-up National Rail services (WMR, LNWR, Avanti, CrossCountry, Chiltern, TfW, EMR) calling at West Midlands stations pass both boarding-contract tests (`in` verdicts recorded). West Midlands Metro also passes both tests and moved to `in` on 5 Sep 2026 after a live service-hours check confirmed both directions return real departures. Severn Valley Railway heritage services at Kidderminster are excluded as non-National-Rail heritage operation (`out-mode` verdict recorded). **All verdicts decided; no silent omissions.**
...
**5 Sep 2026, ~12:11 UTC (05:11 Birmingham, within Metro service hours) — live both-directions check
passed, verdict moved to `in`:** Ran a direct live pull against `fetchMetroStopBoard()` for two
mid-line, two-platform stops — Jewellery Quarter and The Hawthorns. Both returned real GTFS-RT
departures in both directions: southbound to "Edgbaston Village" and northbound to "Wolverhampton
St George's" / "Wolverhampton Station", 8 trips each, none cancelled, `lastUpdate` current. This
rules out the previous 02:00 BST no-service-running result as an hours artifact rather than a merge
bug, and confirms the `stopIds[]` two-platform merge (landed #232, 5 Sep 2026) actually surfaces
