...
| 2026-09-07 | Refinement (Tim, "do what I'd do instead," reversing the 2 Sep 2026 error-surfacing approach for NET): this rule is about not hiding services at stations we show, not about listing stations we can't serve at all. A stop whose operator has no confirmed live-departures feed (Manchester Metrolink, Nottingham NET, Tyne and Wear Metro, Sheffield Supertram, Glasgow Subway, Edinburgh Trams) stays in the catalog — it must reappear the day a feed lands — but is filtered *out of the picker and Near me* rather than being offered and then erroring. The verdict is recorded in the region's coverage note (`lib/cities/<region>/coverage.json`) and a dated line in the pack's own `notes`; the explicit `FeedUnconfirmedError` (PR #329's rider-safe copy) remains the API's behaviour for anyone who addresses the stop directly (e.g. a journey saved before this change). See `docs/jim-brief-no-live-feed-stops-out-of-picker.md`. |
