...
**Backlog:** FB-14  
**Related:** pin session already in `public/app.js` (`nearbySession.pin`, `NEARBY_PIN_HOLD_MS`); leave card `#leave-card`; journey slider under Target; journey face change is **FB-20** (separate brief) — this brief is Near me only  
...
In **Near me**, the rider can **pin** any swiped departure; while pinned, that train stays on the main face **and** the widget, and a **Leave by** card appears with a walk slider and one **Notify me** control.
...
| **C2** | One master **Notify me** under the slider. On ⇒ leave-now ping **and** Live countdown strip. Off ⇒ on-screen leave-by only (no shade). **Default off** — pin alone must not prompt for notification permission. |
...
| Notify me | Toggle under slider. Default **on** when first pinning this session (after permission rules below). |
| Unpinned | Hide leave card + slider + Notify me (journey Leave By rules unchanged when in Journeys mode). |
...
| Notify me | On-screen leave-by | Leave-now ping | Live countdown strip |
|-----------|--------------------|----------------|----------------------|
| **Off** | Yes | No | No |
| **On** | Yes | Yes | Yes |

- First enable (or first pin with default on): reuse existing notification permission gate (`enableLeaveReminders` / same path as journeys). Deny → Notify me stays/returns **off**; leave card + slider still work.  
...
- Leave card placement: existing `#leave-card` region on the main stack (B1) — expand with slider + Notify me only while pinned in Near me.  
- Copy: **Pin** / **Pinned** / **Notify me** / **Time to station** / **Leave by**. Avoid “flag” and avoid “Target train” on this Near me surface.
...
| Area | Where |
|------|--------|
| Pin session | `public/app.js` — `getNearbyPin`, `setNearbyPinFromTrip`, `applyNearbyPinToData`, `isNearbyPinHolding` |
| Leave card | `public/index.html` `#leave-card`; render paths in `public/app.js` |
...
5. **Notify me** off → no leave ping, no strip; on → both (after permission). Deny permission → Notify me off.  
6. Unpin / expiry cancels Near me notifications and hides leave card.  
7. **Widget** shows pinned train (+ leave-by) for the whole hold window, overriding journey active-hours; after clear, widget falls back.  
8. Journey mode Leave By / Target / reminders behaviour unchanged.  
9. No journey `remindMe` / `leaveBeforeMinutes` / strip prefs written by Near me pin flows.
