...
**7 Sep 2026 update (`docs/jim-brief-gtfs-snapshot-freshness.md`) — the "not started" refresh job is now started, and is change-driven, not calendar-driven.** The Vercel cron (`api/health.js` → `lib/gtfs-refresh.js`) now HEAD-probes each Blob-backed city's upstream against a small `gtfs/<city>.json` manifest (ETag/Last-Modified/feed_version, publish time, calendar date range) before downloading anything — an unchanged feed costs one HEAD, same as an unused city; only a changed upstream (or a city the runtime staleness check below has flagged) triggers download-trim-publish and a manifest update. Amsterdam and Rotterdam's shared OVapi feed still OOMs the Vercel Hobby function, so they move to their own GitHub Actions schedule (`.github/workflows/gtfs-refresh.yml` → `scripts/gtfs-refresh-large-feeds.mjs`) running the identical probe-then-publish logic with Actions' larger memory headroom, rather than staying on manual-only refresh. Sydney (Python-only trim) and Auckland/Wellington (agency-zip-at-runtime, no Blob) are unchanged. Separately, the shared board join (`lib/providers/gtfs/board.js`) now measures its own health at request time — resolved share of realtime trip IDs against the snapshot, and calendar coverage of today — and refuses to serve a board it judges stale (`GtfsSnapshotStaleError`, retryable) instead of silently degrading to scheduled-times-presented-as-live. Cities whose realtime feed is a known superset of their static scope (Gold Coast, Adelaide, Amsterdam, Rotterdam, Malmö, Uppsala — a shared regional/national feed carrying modes the trimmed snapshot drops) skip the resolved-share half of that check by design; calendar coverage still applies to every city.
**Related:** `docs/jim-brief-gtfs-fixture-diet.md` (complementary, not a substitute — see §5) · `docs/jim-brief-gtfs-snapshot-freshness.md` (refresh cadence + runtime staleness, 7 Sep 2026) · `lib/providers/gtfs/static-cache.js` · `lib/cities/live-city-api.js` · `lib/providers/registry.js`
