...
**Definition:** v1 mode cut is **metro only** (lines 1, 2, 5, 6 per official STIB map). Services listed below are non-v1 operators and modes that call at in-catalog metro stations. Per rule §6, railway services from a different operator at a shared station are tested like any other service (walk-up + check-in tests); tram and premetro are `out-mode` (different vehicle type).

| Service | Stations | Verdict | Evidence | Real-time API |
| --- | --- | --- | --- | --- |
| SNCB / NMBS domestic intercity, local, S-train (RER) | Gare Centrale / Centraal (1, 5); Gare du Midi / Zuidstation (2, 6); Gare de l'Ouest / Weststation (1, 5) | `in` | No compulsory seat reservation on any SNCB domestic service; walk-up boarding available with valid ticket (ticket must be purchased before boarding as of 1 Jul 2026). [SNCB FAQ travel on board](https://www.belgiantrain.be/en/support/faq/faq-on-board/faq-travel-on-board); [SNCB IC trains ShowMeTheJourney](https://showmethejourney.com/travel-on/train/29-ic-belgium/); [Brussels S-train Wikipedia](https://en.wikipedia.org/wiki/Brussels_S_Train) | GTFS-RT (SNCB Belgian Mobility Portal, updated every 30 seconds) + iRail open API ([iRail Belgian rail real-time](https://api.irail.be/)) |
| Eurostar (UK–Belgium) | Gare du Midi / Zuidstation (2, 6) | `out-checkin` | Mandatory check-in at Channel Terminal, security screening, and border control (Belgian + UK passport control) required before platform access. Closes 30 minutes before departure. [Eurostar Brussels-Midi station](https://www.eurostar.com/rw-en/travel-info/your-trip/stations/brussels-midi) | No public real-time API; operator-proprietary |
| Thalys (Belgium–France–Netherlands) | Gare du Midi / Zuidstation (2, 6) | `out-reservation` | Compulsory seat reservation automatically included with every ticket; cannot board without pre-assigned seat. [Thalys SNCF booking information](https://www.sncf-connect.com/en-en/help/travelling-ouigo) | No public real-time API; operator-proprietary |
| TGV INOUI (Belgium–France) | Gare du Midi / Zuidstation (2, 6) | `out-reservation` | Compulsory seat reservation automatically included with every TGV INOUI ticket. [TGV Brussels-France SNCB International](https://www.b-europe.com/EN/Trains/TGV-Brussels-France) | No public real-time API; SNCF proprietary |
| ICE (Belgium–Germany) | Gare du Midi / Zuidstation (2, 6) | `in` | Seat reservation optional year-round; becomes mandatory only 26 June–16 August on Brussels–Cologne route (ICE 15). Per rule §6, optional reservations classify as `in`. [Deutsche Bahn compulsory reservation FAQ](https://int.bahn.de/en/faq/compulsory-seat-reservation-flexible-fare-europe) | No public real-time API; Deutsche Bahn proprietary |
| OUIGO (France–Belgium low-cost high-speed) | Gare du Midi / Zuidstation (2, 6) | `out-reservation` | Mandatory seat reservation with every ticket; passengers must pass through OUIGO check-in desk (airport-style boarding). Cannot walk up; boarding closes 5 minutes before departure. [OUIGO travel information SNCF Connect](https://www.sncf-connect.com/en-en/help/travelling-ouigo) | No public real-time API; SNCF proprietary |
| Nightjet (Austria ÖBB sleeper, Belgium–Austria) | Gare du Midi / Zuidstation (2, 6) | `out-reservation` | Sleeper service with compulsory bunk reservation. [SNCB International stations Brussels-Midi](https://www.b-europe.com/EN/Stations/Brussels-Midi) | No public real-time API; ÖBB proprietary |
| European Sleeper (Belgium–Austria sleeper) | Gare du Midi / Zuidstation (2, 6) | `out-reservation` | Sleeper service with compulsory bunk reservation. [SNCB International stations Brussels-Midi](https://www.b-europe.com/EN/Stations/Brussels-Midi) | No public real-time API; operator-proprietary |
| Premetro / tram lines 4, 10, 51, 81, 82 (North–South Axis) | De Brouckère (1, 5); Rogier (2, 6); Lemonnier (no metro); Albert (no metro) | `out-mode` | Tram service, excluded by v1 metro-only mode cut. Different vehicle type from metro. Calls at some in-catalog metro stations (De Brouckère, Rogier) as shared underground section. [Brussels premetro Wikipedia](https://en.wikipedia.org/wiki/Brussels_premetro); [North-South Axis Wikipedia](https://en.wikipedia.org/wiki/North%E2%80%93South_Axis) | STIB proprietary JSON (no GTFS-RT); BMC Waiting Time and Vehicle Positions datasets with API key |

**Summary:** SNCB domestic trains at metro stations pass both walk-up and check-in tests (`in`). Eurostar, Thalys, TGV INOUI, OUIGO, and sleeper services fail test 1 (compulsory reservation) or test 2 (check-in / security) and are marked `out-*` accordingly. ICE has optional reservation outside summer peaks and passes the rule (`in`). Premetro/tram lines are excluded by v1 mode cut (`out-mode`). Only SNCB services have public real-time API coverage via GTFS-RT and iRail; international and premetro services use operator-proprietary systems.
