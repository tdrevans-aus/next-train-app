...
/**
 * Rejseplanen live-headsign name for a printed terminus, where it doesn't already contain
 * the printed name as a substring (confirmed live 20 Sep 2026 — see jim-handoff.md "verify
 * live before flip" item 1). Every other terminus in this pack (Vanløse, Vestamager,
 * Klampenborg, Frederikssund, ...) matches the live headsign by fold-substring already
 * (e.g. "Vanløse St. (Metro)" contains "vanløse") and needs no entry here. M2's official
 * printed terminus is "Lufthavnen" (Wikipedia's Copenhagen Metro station list), but the live
 * feed's headsign is "Københavns Lufthavn St. (Metro)" — "lufthavn" without the "en" suffix,
 * so the plain substring test misses it.
 */
const FEED_TERMINUS_ALIASES = new Map(
  [["Lufthavnen", ["Københavns Lufthavn"]]].map(([terminus, aliases]) => [
    terminus,
    aliases.map(foldKey),
  ])
);
 ...
 * by fold-substring match (plus FEED_TERMINUS_ALIASES for the one confirmed-live mismatch
 * above). Returns null (never fabricates) when no known terminus matches — callers should
 * fall back to the raw headsign rather than drop the trip. Rejseplanen headsign format was
 * unverified against a live payload as of the D1 pack (hazard-pack.md item 5,
 * direction-model-memo.md open question 5); confirmed live 20 Sep 2026 for Metro/S-tog — see
 * jim-handoff.md.
...
export function resolveTerminus(rawDestination, routeShortName) {
  ...
  for (const terminus of line.termini) {
    ...
    const aliases = FEED_TERMINUS_ALIASES.get(terminus) ?? [];
...
/**
 * Confirmed live 20 Sep 2026 (jim-handoff.md "verify live before flip" item 1): the real
 * Rejseplanen feed carries EMPTY route_long_name/route_desc for Regionaltog/InterCity/
 * InterCityLyn — the previous route_long_name/route_desc-only heuristic silently dropped all
 * three (verdicted `in` by oracle-clash-report.md's Board eligibility section) from every
 * board. The feed instead puts a short route_short_name code on these routes: "RE"
 * (Regionaltog), "IC" (InterCity), "ICL" (InterCityLyn). EuroCity ("ECE") and České dráhy
 * ("RJ") also carry short codes but are deliberately NOT in this map — both are
 * `out-reservation` per the Board eligibility table and must stay excluded. Öresundståg's
 * route_short_name is a bare corridor number (802/803/804/805, live-confirmed) with no
 * stable meaning of its own; it is still correctly classified below via route_desc, which
 * the live feed does populate with the literal string "Öresundståg" — unlike RE/IC/ICL, that
 * text heuristic was already right, so only Regionaltog/InterCity/InterCityLyn needed a code
 * added here.
 */
const DSB_SHORT_CODES = new Map([
  ["RE", "Regionaltog"],
  ["IC", "InterCity"],
  ["ICL", "InterCityLyn"],
]);
...
/**
 * DSB Regional/InterCity/InterCityLyn/Öresundståg carry no passenger-facing LINE code (no
 * printed M1/A-style letter on the map or platform signage), but Regionaltog/InterCity/
 * InterCityLyn do carry a short internal GTFS route_short_name (RE/IC/ICL, confirmed live —
 * see DSB_SHORT_CODES above), checked first; Öresundståg falls through to the route_desc
 * text heuristic. Returns null when neither matches any in-scope DSB/Öresundståg pattern
...
export function classifyDsbService(trip) {
  const shortCode = String(trip?.routeShortName ?? "").trim().toUpperCase();
  if (DSB_SHORT_CODES.has(shortCode)) {
    return DSB_SHORT_CODES.get(shortCode);
  }
