...
/**
 * "Line + terminus" for Commuter Rail, same convention as mapLineTerminusDestination but for
 * routes with no printed line code (direction-model-memo.md has no Commuter Rail guidance —
 * this follows the memo's recommendation A pattern rather than a new model, flagged for Tim in
 * the PR). `directionId` (0 or 1, MBTA V3 predictions attribute) indexes directly into the
 * route's own two known termini — never derived from a raw trip headsign, so an express
 * variant's unusual headsign text can't leak into the chip.
 */
export function mapCommuterRailDestination(routeId, directionId) {
  ...
  return `${route.longName} + ${terminus}`;
}
