 ...
 * region. Transport for Wales Valley Lines is deliberately NOT wired here
 * — no confirmed public feed of any kind exists for it (see
 * lib/providers/south-wales.js file header and docs/south-wales-d1/
 * jim-handoff.md). Do not add a Valley Lines path off this module.
 *
 * Registry status stays `planned` (this is the pre-flip dogfood wiring
 * pass, docs/jim-brief-south-wales-flip.md) — Mark/Tim's flip call, not
 * made here.
 *
 ...
 * (docs/south-wales-d1/direction-model-memo.md). Directions are derived
 ...
 * in this pipeline.
 *
 * No direction hub anchoring: this catalog is only Cardiff Central (the
 * hub lock) and Severn Tunnel Junction (a flat through-running boundary
 * entry) — there is no intermediate through-station to anchor riders on,
 * unlike West of England's Bristol Temple Meads/Bath Spa or Greater
 * Anglia's Norwich/Thetford/Ely shape. loadDirectionHubs() still runs
 * (returns an empty hub list, a no-op) so this module matches every other
 * UK region's call shape exactly — no south-wales/direction-hubs.json
 * exists and none is needed until a hub candidate is identified.
 */
import {
  SOUTH_WALES_REGION,
