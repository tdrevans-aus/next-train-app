...
export async function getBrusselsDogfoodDirections(station, options = {}) {
  ...
  const directions = [...metroDirections, ...sncbDirections].sort((a, b) =>
...
export async function getBrusselsDogfoodNextTrain({
  ...
  const board = await fetchStationBoard(station, { now, rawResults, irailRawDepartures });
  ...
  const matching = (board.trips ?? []).filter(
    (trip) => tripMatchesMarketingChip(trip, destination) || tripMatchesSncbDirectionChip(trip, destination)
  );
