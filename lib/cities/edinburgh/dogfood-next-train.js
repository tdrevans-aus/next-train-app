/**
 * Edinburgh next-train payload for production Vercel and local dev.
 *
 * Registry status stays `planned` (this is the pre-flip dogfood wiring
 * pass, docs/jim-brief-edinburgh-flip.md) — Mark/Tim's flip call, not made
 * here.
 *
 * Two agencies, two direction models, sharing one dispatcher
 * (lib/providers/edinburgh.js's fetchStationBoard(), reused verbatim — not
 * forked here):
 *
 *  - National Rail (Darwin/OpenLDBWS, regionId "edinburgh"): destination +
 *    operator (e.g. "London Kings Cross (LNER)"), derived live from the
 *    board on every call — no printed National Rail route/line map exists,
 *    same reasoning and same live-derivation shape as every other UK NR
 *    region (East Midlands, North East, South Yorkshire, ...). Throws
 *    MissingDarwinTokenError until DARWIN_LDB_TOKEN exists — not caught
 *    here, same as every other UK region. SINGLE hub-lock shape (Edinburgh
 *    Waverley, EDB) with two through-running satellites (Haymarket, HYM;
 *    Slateford, SLA) — structurally different from Glasgow's two
 *    independent termini, see lib/providers/edinburgh.js file header.
 *
 *  - Edinburgh Trams T50: no confirmed real-time feed exists (TfE Open Data
 *    API closed/inactive, no successor confirmed — see
 *    lib/providers/edinburgh.js file header). lib/cities/edinburgh/
 *    tram-directions.js's line+terminus labels are a *design* artifact
 *    (used by the QA gate to prove the catalog/direction-model shape), not
 *    a confirmed schedule — treating them as dogfood output would
 *    fabricate live-looking data from an unconfirmed feed.
 *    fetchTramStopBoard() throws EdinburghTramsFeedUnverifiedError
 *    unconditionally; this module does not catch it, does not fall back to
 *    the static label list, and does not fabricate a schedule. Same shape
 *    as East Midlands' NET layer, Greater Manchester's Metrolink layer,
 *    South Yorkshire's Supertram layer, and North East's Metro layer.
 *
 * Edinburgh Waverley is a doNotGroup hub lock: the Trams "Waverley area"
 * stop and the National Rail station are two separate catalog entries with
 ...
 * metro (same order as lib/providers/edinburgh.js's own dispatcher), so an
 * ambiguous bare "Edinburgh Waverley" lookup resolves to the National Rail
 * layer; callers that need the Trams layer must resolve/pass the metro
 * catalog entry explicitly (mode: "metro"), same disambiguation the catalog
 * itself already requires via resolveCatalogEntry(name, mode). Haymarket
 * carries the same two-catalog-entries-by-mode shape (Trams stop vs
 * National Rail station), kept distinct by mode-scoped lookup only.
 *
 * No direction-hubs.json ships for this region — the D1 pack names no
 * intermediate through-station candidate for a hub chip (Haymarket and
 * Slateford are through-running satellites of the Waverley hub-lock
 * itself, not feeders into it). loadDirectionHubs() still runs (returns an
 ...
 * termini via the national name->CRS index (lib/cities/uk/rail-crs-index.js)
 * rather than only the region's own 3-station rail catalog.
 */
import {
  EDINBURGH_TIME_ZONE,
  EDINBURGH_REGION,
...
export function listEdinburghDogfoodStations() {
  // Jim brief: server must not parse large GTFS fixtures for city-stations.
  // Coords belong on stations.json. mode is included because Edinburgh
  // Waverley's (and Haymarket's) doNotGroup hub lock needs it to
  // disambiguate the two catalog entries with the same name.
 ...
 *
 * Edinburgh Waverley's (and Haymarket's) doNotGroup hub lock means the same
 * printed name resolves to two different catalog entries by mode — passing
 * the resolved name back into fetchStationBoard() would re-run the SAME
 * ambiguous, rail-first resolution the provider's own dispatcher uses (see
 * lib/providers/edinburgh.js), silently discarding an explicit metro
...
async function fetchBoardForMode(station, mode) {
  ...
  if (mode === "metro") {
    return fetchTramStopBoard(station);
  }
 ...
 * direction-hubs.json exists — see file header). Trams: always throws
 * EdinburghTramsFeedUnverifiedError — see file header; not caught here.
 * @param {string} station
 * @param {{ mode?: "train"|"metro" }} [options]
 */
export async function getEdinburghDogfoodDirections(station, { mode } = {}) {
  ...
  const directions =
    resolvedMode !== "metro" && entry?.crs
      ? applyDirectionHubs(sorted, entry.crs, loadDirectionHubs(EDINBURGH_REGION).hubs)
      : sorted;
  return {
    directions,
    source: "edinburgh-darwin-live",
  };
}

/**
 * Pure routing decision for getEdinburghDogfoodNextTrain() — exported
 * separately (no fetch inside it) so the QA gate can assert the branching
 * table with a fixture, without a Darwin token. Thin wrapper over the
 * shared lib/cities/uk/direction-hubs.js's planUkNextTrainFetch(), with the
 ...
 * @param {object[]} hubs Loaded hub configs (loadDirectionHubs(EDINBURGH_REGION).hubs).
 */
export function planEdinburghNextTrainFetch(entry, resolvedMode, destination, hubs) {
...
export async function getEdinburghDogfoodNextTrain({
  station,
  mode,
  destination,
  destinationLabel,
  ...
  const entry = resolveCatalogEntry(station, mode);
  const resolvedMode = mode ?? entry?.mode;
  const plan = planEdinburghNextTrainFetch(
    entry,
    resolvedMode,
    destination,
    loadDirectionHubs(EDINBURGH_REGION).hubs
  );
  ...
  if (plan.kind === "hub") {
    ...
    return buildEdinburghNextTrainResponse({
      board,
      station,
      destination,
      destinationLabel,
  ...
  // Anything else (unresolvable destination, or the Trams layer — which
  // throws EdinburghTramsFeedUnverifiedError inside fetchBoardForMode,
...
/**
 * Shared response builder for getEdinburghDogfoodNextTrain()'s three
 ...
 */
function buildEdinburghNextTrainResponse({
  board,
  station,
  destination,
  destinationLabel,
