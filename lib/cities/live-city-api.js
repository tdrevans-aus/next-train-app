...
async function directionsFor(cityId, station) {
  ...
  if (cityId === "boston") {
    // Subway line+terminus chips plus Commuter Rail line+terminus chips at the five
    // board-eligible stations (docs/boston-d1/oracle-clash-report.md Board eligibility
    // section) — both come from marketingLabelsForStation's static catalog/route tables, no
    // live call needed to enumerate chips. Not in MULTI_CITY_IDS yet — registry status stays
    // "planned" until Mark/Tim flip it (docs/boston-d1/jim-handoff.md). Safe to wire the
    // switch-case ahead of that: production routes gate on assertCityLive() first, not on
    // this list.
    const { marketingLabelsForStation } = await import("./boston/marketing-directions.js");
    return { directions: marketingLabelsForStation(station), source: "boston-marketing-ends" };
  }
  ...
  if (cityId === "rest-of-england") {
    // National Rail: destination+operator derived live from Darwin, same approach
    // as every other UK NR region. Internal catch-all for the 436 English
    // stations UK station fill phase 2a's fifteen named regions did not claim —
    // no hub lock, no doNotGroup case in this catalog (see
    // lib/providers/rest-of-england.js file header). Not in MULTI_CITY_IDS yet —
    // registry status stays "planned" until Mark/Tim flip it
    // (docs/jim-brief-uk-station-fill-phase2b.md). Safe to wire the switch-case
    // ahead of that: production routes gate on assertCityLive() first, not on
    // this list.
    const { getRestOfEnglandDogfoodDirections } = await import(
      "./rest-of-england/dogfood-next-train.js"
    );
    return getRestOfEnglandDogfoodDirections(station);
  }
  if (cityId === "glasgow") {
  ...
  if (cityId === "brussels") {
    // Single agency (STIB/MIVB metro 1/2/5/6), directions derived from the
    // printed line map (same shape as Malmö/Uppsala) — see
    // lib/cities/brussels/dogfood-next-train.js file header for why SNCB is
    // NOT wired here despite an `in` board-eligibility verdict. Not in
    // MULTI_CITY_IDS yet — registry status stays "planned" until Mark/Tim
    // flip it (docs/brussels-d1/jim-handoff.md). Safe to wire the
    // switch-case ahead of that: production routes gate on assertCityLive()
    // first, not on this list.
    const { getBrusselsDogfoodDirections } = await import("./brussels/dogfood-next-train.js");
    return getBrusselsDogfoodDirections(station);
  }
  if (cityId === "copenhagen") {
    // Metro M1-M4 + S-tog + DSB Regional/InterCity/InterCityLyn + Öresundståg, all via the
    // shared Rejseplanen national platform (lib/providers/rejseplanen.js), Copenhagen a
    // config over it (lib/providers/copenhagen.js) per docs/denmark-ledger.md. Directions are
    // derived live from the board's own already-computed chips, not a static marketing-
    // directions list — see lib/cities/copenhagen/dogfood-next-train.js file header. Schedule-
    // only (no Rejseplanen API 2.0/SIRI-ET key wired). Flipped live 26 Sep 2026 (Tim's decision,
    // docs/copenhagen-d1/mark-qa-note.md + docs/copenhagen-d1/ci-live-evidence.md) — in
    // MULTI_CITY_IDS.
    const { getCopenhagenDogfoodDirections } = await import("./copenhagen/dogfood-next-train.js");
    return getCopenhagenDogfoodDirections(station);
  }
  if (cityId === "chicago") {
    // CTA 'L', eight color lines, directions are static "line + terminus" chips (same shape
    // as Boston/BART) from marketingLabelsForStation's catalog/line tables, no live call
    // needed to enumerate chips. Not in MULTI_CITY_IDS yet — registry status stays "planned"
    // until Mark/Tim flip it (docs/chicago-d1/jim-handoff.md). Safe to wire the switch-case
    // ahead of that: production routes gate on assertCityLive() first, not on this list.
    const { marketingLabelsForStation } = await import("./chicago/marketing-directions.js");
    return { directions: marketingLabelsForStation(station), source: "chicago-marketing-ends" };
  }
  if (cityId === "washington") {
    // WMATA Metrorail, six colour lines, directions are static "Line + terminus" chips (same
    // shape as Chicago/Boston/BART) from marketingLabelsForStation's catalog/line tables, no
    // live call needed to enumerate chips. Flipped live 25 Sep 2026
    // (docs/washington-d1/mark-qa-note.md) — in MULTI_CITY_IDS.
    const { marketingLabelsForStation } = await import("./washington/marketing-directions.js");
    return { directions: marketingLabelsForStation(station), source: "washington-marketing-ends" };
  }
  if (cityId === "melbourne") {
    // Metro Trains + V/Line walk-up services, GTFS-RT static-join over the Transport Victoria
    // Open Data Portal. Directions derived live from the board's own already-computed
    // "Line + terminus [via City Loop]" / "V/Line {terminus}" chips, not a static marketing
    // list — the loop/direct suffix and short-turn termini are per-trip facts, same approach
    // as Copenhagen — see lib/cities/melbourne/dogfood-next-train.js file header. Not in
    // MULTI_CITY_IDS yet — registry status stays "planned" until Mark/Tim flip it
    // (docs/melbourne-d1/jim-handoff.md). Safe to wire the switch-case ahead of that:
    // production routes gate on assertCityLive() first, not on this list.
    const { getMelbourneDogfoodDirections } = await import("./melbourne/dogfood-next-train.js");
    return getMelbourneDogfoodDirections(station);
  }
...
export async function getMultiCityNextTrain(cityId, rawConfig) {
  const id = String(cityId || "").trim().toLowerCase();
  // jim-brief-direction-label-aliases-server-side.md: canonicalize a legacy direction label
  // (an old client bundle, or a stale saved journey) before it reaches any city's matching
  // logic, and let the canonical label flow through to the response so the client self-heals.
  const config = canonicalizeDirectionConfig(id, rawConfig);
  if (id === "sydney") {
  ...
  if (id === "uk-london-tfl") {
    const { fetchStopBoard, findTerminatingArrivals } = await import("../providers/uk-tfl.js");
    const { buildNextTrainResponse, pickUpcomingProviderTrips } =
      await import("../train-times-core.js");
    ...
    // Terminating-arrivals detection (docs/jim-brief-terminus-no-published-departures.md) only
    // does anything when the chosen direction resolves empty — a normal board with departures
    // is untouched. trips are already internal-shape (providerTripToInternal ran in
    // parseTflArrival/parseTflArrivalDeparture), but pickUpcomingProviderTrips expects provider
    // shape and re-runs providerTripToInternal itself; findTerminatingArrivals works directly
    // on the internal trips already in `trips`.
    const terminatingTrips =
      upcoming.length === 0
        ? findTerminatingArrivals(trips, board.stationName, config.destination, now)
        : [];
  ...
  if (id === "boston") {
    ...
    const { getBostonDogfoodNextTrain } = await import("./boston/dogfood-next-train.js");
    return getBostonDogfoodNextTrain(config);
  }
  ...
  if (id === "rest-of-england") {
    // Not in MULTI_CITY_IDS yet — see the directionsFor() note above.
    const { getRestOfEnglandDogfoodNextTrain } = await import(
      "./rest-of-england/dogfood-next-train.js"
    );
    return getRestOfEnglandDogfoodNextTrain(config);
  }
  if (id === "glasgow") {
    // Not in MULTI_CITY_IDS yet — see the directionsFor() note above.
  ...
  if (id === "brussels") {
    // Not in MULTI_CITY_IDS yet — see the directionsFor() note above.
    const { getBrusselsDogfoodNextTrain } = await import("./brussels/dogfood-next-train.js");
    return getBrusselsDogfoodNextTrain(config);
  }
  if (id === "copenhagen") {
    // Flipped live 26 Sep 2026 — see the directionsFor() note above. Schedule-only (no
    // Rejseplanen API 2.0/SIRI-ET key), realtime: false, per docs/copenhagen-d1/mark-qa-note.md.
    const { getCopenhagenDogfoodNextTrain } = await import("./copenhagen/dogfood-next-train.js");
    return getCopenhagenDogfoodNextTrain(config);
  }
  if (id === "chicago") {
    // Not in MULTI_CITY_IDS yet — see the directionsFor() note above. CTA_TRAIN_TRACKER_KEY is
    // not registered yet, so this throws MissingCtaTrainTrackerKeyError until Tim signs up.
    const { getChicagoDogfoodNextTrain } = await import("./chicago/dogfood-next-train.js");
    return getChicagoDogfoodNextTrain(config);
  }
  if (id === "washington") {
    // Flipped live 25 Sep 2026 (docs/washington-d1/mark-qa-note.md) — in MULTI_CITY_IDS.
    // WMATA_API_KEY is required; fetchStationBoard() throws MissingWmataApiKeyError if unset.
    const { getWashingtonDogfoodNextTrain } = await import("./washington/dogfood-next-train.js");
    return getWashingtonDogfoodNextTrain(config);
  }
  if (id === "melbourne") {
    // Not in MULTI_CITY_IDS yet — see the directionsFor() note above.
    const { getMelbourneDogfoodNextTrain } = await import("./melbourne/dogfood-next-train.js");
    return getMelbourneDogfoodNextTrain(config);
  }
  throw new Error(`Unknown live city: ${cityId}`);
}
