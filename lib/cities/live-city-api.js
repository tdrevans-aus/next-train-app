...
/** @typedef {"sydney"|"brisbane"|"adelaide"|"uk-london-tfl"|"amsterdam"|"rotterdam"|"vancouver"|"canberra"|"gold-coast"|"newcastle"|"auckland"} MultiCityId */

/** @type {MultiCityId[]} */
export const MULTI_CITY_IDS = ["sydney", "brisbane", "adelaide", "uk-london-tfl", "amsterdam", "rotterdam", "vancouver", "canberra", "gold-coast", "newcastle", "auckland"];
...
async function directionsFor(cityId, station) {
  ...
  if (cityId === "goteborg") {
    // Adapter-ready but still `planned` — deliberately NOT in MULTI_CITY_IDS.
    // Production API routes gate on assertCityLive + isMultiCity first, so this
    // branch is unreachable from the live app until Tim adds "goteborg" to
    // MULTI_CITY_IDS and flips the registry. It exists so QA and the bundled
    // city-directions build can exercise the exact production path pre-flip.
  ...
  if (cityId === "stockholm") {
    // Adapter-ready but still `planned` — deliberately NOT in MULTI_CITY_IDS.
    // Production API routes gate on assertCityLive + isMultiCity first, so this
    // branch is unreachable from the live app until Tim adds "stockholm" to
    // MULTI_CITY_IDS and flips the registry. It exists so QA and the bundled
    // city-directions build can exercise the exact production path pre-flip.
...
export async function getMultiCityNextTrain(cityId, config) {
  ...
  if (id === "goteborg") {
    // Schedule-only board (no Trafiklab TripUpdates for vt). Unreachable from
    // production until the live flip — see directionsFor("goteborg") note.
    const { getGoteborgDogfoodNextTrain } = await import("./goteborg/dogfood-next-train.js");
    return getGoteborgDogfoodNextTrain(config);
  }
  if (id === "stockholm") {
    // Realtime SL Transport board (expected times, no key). Unreachable from
    // production until the live flip — see directionsFor("stockholm") note.
