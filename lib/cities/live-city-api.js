...
async function directionsFor(cityId, station) {
  ...
  if (cityId === "copenhagen") {
    ...
    // only (no Rejseplanen API 2.0/SIRI-ET key wired). Not in MULTI_CITY_IDS yet — registry
    // status stays "planned" until Mark/Tim flip it (docs/copenhagen-d1/jim-handoff.md). Safe
    // to wire the switch-case ahead of that: production routes gate on assertCityLive() first,
    // not on this list.
  ...
  if (cityId === "washington") {
    ...
    // live call needed to enumerate chips. Not in MULTI_CITY_IDS yet — registry status stays
    // "planned" until Mark/Tim flip it (docs/washington-d1/jim-handoff.md). Safe to wire the
    // switch-case ahead of that: production routes gate on assertCityLive() first, not on this
    // list.
...
export async function getMultiCityNextTrain(cityId, rawConfig) {
  ...
  if (id === "washington") {
    // Not in MULTI_CITY_IDS yet — see the directionsFor() note above. WMATA_API_KEY is not
    // registered yet, so this throws MissingWmataApiKeyError until Tim signs up.
