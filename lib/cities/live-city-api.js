...
/** @typedef {"sydney"|"brisbane"|"adelaide"|"uk-london-tfl"|"amsterdam"|"rotterdam"|"vancouver"|"canberra"|"gold-coast"|"newcastle"|"auckland"|"stockholm"|"goteborg"|"wellington"|"malmo"|"uppsala"|"oslo"} MultiCityId */

/** @type {MultiCityId[]} */
export const MULTI_CITY_IDS = ["sydney", "brisbane", "adelaide", "uk-london-tfl", "amsterdam", "rotterdam", "vancouver", "canberra", "gold-coast", "newcastle", "auckland", "stockholm", "goteborg", "wellington", "malmo", "uppsala", "oslo"];
// NOTE (Jim, 30 Aug 2026): "oslo" deliberately NOT added here yet — this array is asserted by
// qa/live-city-lists-sync.mjs to equal exactly the registry's status==="live" cities (minus
// perth), and Oslo's registry status stays "planned" until Tim/Mark's flip-PR (see
// lib/providers/registry.js, docs/oslo-d1/jim-handoff.md). Precedent: hong-kong/osaka (also
// still planned) have zero entries here for the same reason. The oslo dispatch cases below
// (directionsFor / getMultiCityNextTrain) are wired and tested (qa/oslo-dogfood-gate.mjs) via
// direct getMultiCityDirections/getMultiCityNextTrain calls, so add "oslo" to this array (and
// the MultiCityId typedef above) as part of the same commit that flips oslo's registry status
// to "live" — matching how malmo/uppsala/goteborg's MULTI_CITY_IDS entries landed together with
// their status flip, not before it.
