...
export const MARKETING_ENDS = {
  ...
  // docs/jim-brief-sydney-intercity-fill.md). Walk-up only — booked
  // XPT/Xplorer/coach services never get a chip; see
...
export const CHIP_HEADSIGN_GROUPS = {
  ...
  "BMT Lithgow": ["Lithgow"],
  "CCN Central": ["Central"],
  "CCN Newcastle Interchange": ["Newcastle Interchange", "Newcastle"],
  "SCO Central": ["Central"],
  "SCO Bomaderry": ["Bomaderry", "Nowra"],
  "SHL Central": ["Central"],
  "SHL Goulburn": ["Goulburn"],
  "HUN Dungog": ["Dungog"],
  "HUN Scone": ["Scone"],
};
...
function chipLinePrefix(chip) {
  return (
    String(chip || "")
      .trim()
      .match(/^(T\d+|M\d+|BMT|CCN|SCO|SHL|HUN)/i)?.[1]
      ?.toUpperCase() ?? ""
  );
}
...
function chipTerminus(chip) {
  return String(chip || "")
    .trim()
    .replace(/^(T\d+|M\d+|BMT|CCN|SCO|SHL|HUN)\s+/i, "")
    .trim();
}
