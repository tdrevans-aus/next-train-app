/**
 * Rest of Wales next-train payload for production Vercel and local dev.
 *
 * National Rail only (Darwin/OpenLDBWS via lib/providers/uk-darwin.js,
 * regionId "rest-of-wales") — genuinely Darwin-or-nothing, no static GTFS
 * fallback exists for this feed at all (see lib/providers/rest-of-wales.js
 ...
 * region. Transport for Wales real-time feed status through Darwin is
 * UNCONFIRMED even once the account-level token exists (see
 * lib/providers/rest-of-wales.js file header and docs/rest-of-wales-d1/
 * jim-handoff.md) — this module does not paper over that; a board call
 * either succeeds or throws, nothing is invented in between.
 *
 * Registry status stays `planned` (this is the pre-flip dogfood wiring
 * pass, docs/jim-brief-rest-of-wales-flip.md) — Mark/Tim's flip call, not
 * made here.
 *
 * Direction model is destination + operator (e.g. "Holyhead (TfW)"),
 * matching how National Rail departure boards actually present
 * (docs/rest-of-wales-d1/direction-model-memo.md). Directions are derived
 ...
 * in this pipeline.
 *
 * Hub lock is Wrexham General (WRX) only, but NO direction-hubs.json ships
 * for this region — unlike Greater Anglia's Thetford/Ely -> Norwich
 * anchoring, the D1 pack names no intermediate through-station that prints
 * "Wrexham General" as a destination without it being the trip's own
 * terminus (see hazard-pack.md H6, jim-handoff.md). loadDirectionHubs()
 * still runs (returns an empty hub list, a no-op) so this module matches
 * every other UK region's call shape exactly. The four doNotGroup
 * candidates flagged in the D1 pack (Wrexham General route directions,
 * Carmarthen branch-vs-through-running, Whitland three-way branch,
 * Machynlleth Aberystwyth/Pwllheli split) are a separate concept from hub
 * anchoring and are likewise not built — no platform/service-pattern detail
 * to build an enforced rule from.
 */
import {
  REST_OF_WALES_REGION,
  REST_OF_WALES_TIME_ZONE,
  fetchStationBoard,
  listCatalogStations,
  resolveCatalogEntry,
} from "../../providers/rest-of-wales.js";
...
/**
 * "Holyhead (TfW)" — matches how Darwin/real departure boards present.
...
export function listRestOfWalesDogfoodStations() {
 ...
 * rest-of-wales/direction-hubs.json exists — see file header).
 * @param {string} station
 */
export async function getRestOfWalesDogfoodDirections(station) {
  ...
  const directions = entry?.crs
    ? applyDirectionHubs(sorted, entry.crs, loadDirectionHubs(REST_OF_WALES_REGION).hubs)
    : sorted;
  return {
    directions,
    source: "rest-of-wales-darwin-live",
  };
}

/**
 * Pure routing decision for getRestOfWalesDogfoodNextTrain() — exported
 ...
 *    in the region's own 17-station catalog or the national index.
 ...
 * @param {object[]} hubs Loaded hub configs (loadDirectionHubs(REST_OF_WALES_REGION).hubs).
 */
export function planRestOfWalesNextTrainFetch(entry, destination, hubs) {
  return planUkNextTrainFetch(entry, "train", destination, hubs, {
    regionId: REST_OF_WALES_REGION,
    resolveDestinationCrs: resolveCrsForName,
  });
}

export async function getRestOfWalesDogfoodNextTrain({
  station,
  destination,
  destinationLabel,
  leaveBeforeMinutes,
  ...
  const entry = resolveCatalogEntry(station);
  const plan = planRestOfWalesNextTrainFetch(
    entry,
    destination,
    loadDirectionHubs(REST_OF_WALES_REGION).hubs
  );
  ...
  if (plan.kind === "hub") {
    ...
    return buildRestOfWalesNextTrainResponse({
      board,
      station,
      destination,
      destinationLabel,
...
/**
 * Shared response builder for getRestOfWalesDogfoodNextTrain()'s three
 ...
 */
function buildRestOfWalesNextTrainResponse({
  board,
  station,
  destination,
  destinationLabel,
  ...
  return buildNextTrainResponse({
    ...
    timeZone: REST_OF_WALES_TIME_ZONE,
  });
}
