 ...
 *    unset. Credentials exist (FB-48; live both-directions boards proven
 *    5 Sep 2026, FB-53) — the keys are per-environment, so any deployment
 *    without them still sees the error rather than a fabricated board. This
 *    is not a "feed unconfirmed" situation like East Midlands' NET tram (the
 *    TfWM GTFS-RT feed itself is real and documented); it is a config gap.
...
import {
  loadDirectionHubs,
  applyDirectionHubs,
} from "./direction-hubs.js";
import { planUkNextTrainFetch } from "../uk/direction-hubs.js";
...
export async function getUkWestMidlandsDogfoodDirections(station, { mode } = {}) {
  ...
  // Hubs are rail-only, and only meaningful for a resolved rail catalog
  // entry (metro stops have no CRS) — mode "metro" never consults the hub
  // file, per the brief.
  const directions =
    resolvedMode !== "metro" && entry?.crs
      ? applyDirectionHubs(sorted, entry.crs, loadDirectionHubs(UK_WEST_MIDLANDS_REGION).hubs)
      : sorted;
  return {
    directions,
...
/**
 * Pure routing decision for getUkWestMidlandsDogfoodNextTrain() — exported
 ...
 * (FB-51 lift) bound to the West Midlands region, with no national-index
 * fallback — West Midlands' exact-chip path only ever needed the region
 * catalog (see the shared helper's own doc comment for the full table):
 *  - "hub": destination matches a hub label for this station's CRS.
 *  - "exact": destination's own destination part resolves to a region CRS.
 *  - "undirected": anything else (e.g. an out-of-region destination) — the
 *    current undirected-fetch-then-client-filter path, unchanged.
 ...
 */
export function planUkWestMidlandsNextTrainFetch(entry, resolvedMode, destination, hubs) {
  return planUkNextTrainFetch(entry, resolvedMode, destination, hubs, {
    regionId: UK_WEST_MIDLANDS_REGION,
  });
}
...
export async function getUkWestMidlandsDogfoodNextTrain({
  ...
  const entry = resolveCatalogEntry(station, mode);
  const resolvedMode = mode ?? entry?.mode;
  const plan = planUkWestMidlandsNextTrainFetch(
    entry,
    resolvedMode,
    destination,
    loadDirectionHubs(UK_WEST_MIDLANDS_REGION).hubs
  );
  ...
  if (plan.kind === "hub") {
    ...
    return buildHubNextTrainResponse({
      board,
      station,
      destination,
      destinationLabel,
  ...
  if (plan.kind === "exact") {
    // Exact chip whose printed destination is a region-catalog station:
    // fetch server-side filtered to that CRS, then keep the operator match
    // client-side (Darwin's filterCrs doesn't know about operator).
    const board = await fetchRegionalDepartureBoard(entry.name ?? station, plan.filterCrs, {
      regionId: UK_WEST_MIDLANDS_REGION,
      numRows: 15,
    });
  ...
  // Anything else (e.g. an out-of-region destination): current undirected
  // path, unchanged.
  const board = await fetchBoardForMode(entry?.name ?? station, resolvedMode);
  const matching = (board.trips ?? []).filter((trip) => directionChip(trip) === destination);
  return buildHubNextTrainResponse({
...
/**
 * Shared response builder for getUkWestMidlandsDogfoodNextTrain()'s three
 ...
 * field, carried through buildTripPayload() in train-times-core.js. For a
 * hub chip this is how a row can show "Birmingham · to Stratford-upon-Avon";
 * for an exact chip printedDestination equals destination and the render
 * can omit showing it.
 */
function buildHubNextTrainResponse({
  board,
  station,
  destination,
  destinationLabel,
