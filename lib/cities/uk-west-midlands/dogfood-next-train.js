...
/**
 * Resolves an exact chip's destination part to a region-catalog CRS, or
 * null if it doesn't resolve (e.g. an out-of-region destination like
 * "London Euston (Avanti West Coast)") — the caller falls back to the
 * undirected path in that case.
 * @param {string} destination
 */
function resolveExactChipCrs(destination) {
  const destPart = chipDestinationPart(destination);
  if (!destPart) {
    return null;
  }
  const resolved = resolveRailEntry(destPart, UK_WEST_MIDLANDS_REGION);
  return resolved?.crs ?? null;
}
...
export function planUkWestMidlandsNextTrainFetch(entry, resolvedMode, destination, hubs) {
  // Hubs are rail-only; a resolved rail catalog entry (has a CRS) is
  // required before consulting the hub file — mode "metro" never does.
  if (resolvedMode !== "metro" && entry?.crs) {
    ...
    const exactCrs = resolveExactChipCrs(destination);
    if (exactCrs) {
      return { kind: "exact", filterCrs: exactCrs };
    }
  }
