 ...
 *
 * Static: a Vercel Blob snapshot (gtfsFixtureBlobUrl("uppsala")) published from Trafiklab
 * GTFS Regional `ul` (TRAFIKLAB_API_KEY) by scripts/publish-gtfs-snapshot-to-blob.mjs — never
 * fetched from Trafiklab directly on the request path, which 429'd under normal cold-start
 * traffic (see docs/jim-brief-sweden-static-429-and-key-leak.md).
...
export async function loadUppsalaStatic() {
  return loadGtfsStatic({
    url: gtfsFixtureBlobUrl("uppsala"),
 ...
 * @param {{ now?: Date, horizonMinutes?: number, loadStatic?: () => Promise<object> }} [options]
 *   `loadStatic` defaults to `loadUppsalaStatic` (the Vercel Blob snapshot); an override lets
 *   QA exercise the wrapper offline, same seam as malmo.js
 *   (docs/jim-brief-malmo-planned-closure-empty-board.md, round 3).
 */
export async function fetchStationBoard(stationIdOrName, options = {}) {
  ...
  const board = await fetchGtfsRealtimeBoard(stationIdOrName, {
    loadStatic: options.loadStatic ?? loadUppsalaStatic,
    resolveStopIds,
    resolveCatalogEntry,
    tripUpdatesUrl,
    timeZone: UPPSALA_TIME_ZONE,
    cityId: "uppsala",
    // UL's per-operator Trafiklab feed is multimodal (bus + rail); the
    // static snapshot here is trimmed to rail route_types only, so most
