...
async function loadNewcastleStatic() {
  // gtfsFixtureBlobUrl() is our own public Vercel Blob mirror of a trimmed TfNSW snapshot,
  // not the live TfNSW endpoint — it needs no auth, and Vercel Blob's public storage
  // returns 401 (surfaced as "GTFS static download failed") if it receives an
  // Authorization header it doesn't recognise. Only the RT fetch needs TFNSW_API_KEY.
