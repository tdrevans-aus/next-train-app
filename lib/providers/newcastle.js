...
export async function loadNewcastleStatic() {
  // The static snapshot is served from the public Vercel Blob store, which
  // rejects any Authorization header it doesn't recognise — sending the TfNSW
  // `apikey` header here turned every production load into a 401 (found in
  // the 6 Sep 2026 pre-launch sweep). The TfNSW key is only for the RT feed.
  return loadGtfsStatic({
    url: gtfsFixtureBlobUrl("newcastle"),
    routeTypes: ["0"],
