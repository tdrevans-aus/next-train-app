 ...
 *
 * Live-only boards: a trip the GTFS-RT feed never mentions is dropped, never
 * backfilled from the static timetable as if it were live
 * (docs/jim-brief-boston-subway-live-predictions.md's lesson — a scheduled board
 * must never be presented as live). A TripUpdate that names the trip confirms
 * it; stop_time_updates are optional and often cover only some stops, so an
 * unmentioned stop on a confirmed trip keeps its scheduled time. See
 * tripHasRealtimeConfirmation() below.
 *
 * Direction labels: terminus only, Perth style (e.g. "Williamstown"), except at the
 * shared hub terminus Flinders Street, where the line name disambiguates
 * ("Flinders Street (Williamstown Line)") — relabelled 22 Sep 2026,
 * docs/jim-brief-melbourne-direction-labels-perth-style.md. A "via City Loop" suffix
 * is appended only at Flinders Street / Southern Cross / Flagstaff / Melbourne
 * Central / Parliament, derived per-trip from the *static* stop-sequence (never
 * headsign alone, never a time-of-day rule). A trip terminating at the station being
 * viewed is dropped as an arrival, not a departure. See
 * lib/cities/melbourne/direction-labels.js and
...
/**
 * Live-only rule: a trip the RT feed never mentions is a scheduled-time guess,
 * not a live fact — drop it rather than present a timetable time as live
 * (docs/jim-brief-boston-subway-live-predictions.md).
 *
 * A TripUpdate confirms the whole trip. GTFS-RT stop_time_updates are optional;
 * feeds often include only the stops whose time changed. Requiring a stop-level
 * update at this exact stop hid a trip that was live one station along (Camberwell
 * showed the 00:01 to Alamein while Ashburton, which that trip calls at, was empty).
 * An unmentioned stop on a confirmed trip keeps the scheduled time already
 * computed in buildBoardForStops, shifted by any trip-level delay.
 */
export function tripHasRealtimeConfirmation(trip, realtimeIndex) {
  const key = `${trip.tripId}:${trip.stopId}`;
  if (realtimeIndex.stopUpdates.has(key) || realtimeIndex.tripDelaySec.has(trip.tripId)) {
    return true;
  }
  return realtimeIndex.tripIds instanceof Set && realtimeIndex.tripIds.has(trip.tripId);
}
...
export async function fetchStationBoard(stationIdOrName, options = {}) {
  ...
  // departure — never a boardable direction from here (docs/jim-brief-
  // melbourne-direction-labels-perth-style.md item 3).
  trips = trips.filter((trip) => !isTerminatingAtStation({ destination: trip.destination, stationName }));
