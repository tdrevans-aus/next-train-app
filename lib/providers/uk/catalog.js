 ...
 * Region ids: uk-west-midlands, uk-ellesmere-port, uk-london-tfl.
...
export const UK_REGION_IDS = ["uk-west-midlands", "uk-ellesmere-port", "uk-london-tfl"];
...
for (const region of regionsIndex.regions ?? []) {
  const catalog = loadJson(join(UK_PROVIDERS_ROOT, region.catalogPath));
  regionCatalogs.set(region.id, { meta: region, catalog });
}
...
export function getRegion(regionId) {
  ...
  const pack = regionCatalogs.get(id);
  if (!pack) {
    return null;
  }
...
function catalogStops(regionId) {
  const pack = regionCatalogs.get(String(regionId || "").trim().toLowerCase());
