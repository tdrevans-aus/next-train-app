 ...
 * @param {{ now?: Date, horizonMinutes?: number, staticData?: object }} [options]
 *   `staticData` — an already-loaded GTFS static snapshot (same shape `loadBrusselsStatic()`
 *   returns). When supplied the live STIB fetch is skipped entirely, so an offline gate
 *   (qa/brussels-planned-gate.mjs, fed by `loadGtfsStaticFromDirectory`) can drive this exact
 *   filter/map pipeline against a committed fixture instead of copy-pasting it. Production
...
export async function fetchStationBoard(stationIdOrName, options = {}) {
  ...
  const staticData = options.staticData ?? (await loadBrusselsStatic());
