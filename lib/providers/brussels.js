 ...
 *
 * SNCB/NMBS domestic rail at Gare Centrale / Gare du Midi / Gare de l'Ouest is ruled `in` by
 * docs/brussels-d1/oracle-clash-report.md's Board eligibility section, with a confirmed free
 * real-time source (iRail liveboard API) — deliberately NOT wired here (second provider
 * surface + station-name map + doNotGroup-by-mode catalog split — genuine follow-up scope,
 * see docs/brussels-d1/jim-handoff.md). Recorded in registry.js `notes` so Mark's QA holds
 * the flip on this gap.
 */
 ...
 * @param {{ now?: Date, rawResults?: object[], noCache?: boolean }} [options]
...
export async function fetchStationBoard(stationIdOrName, options = {}) {
  ...
  if (options.rawResults) {
    ...
    // attempted — never a silent timetable fallback.
    try {
  ...
  const trips = mapWaitingTimesResults(rawResults, stationKey);
