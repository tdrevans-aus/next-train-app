 ...
 * excludeOperators option, no new plumbing): St Pancras International
 * excludes Eurostar (out-checkin); Paddington excludes Night Riviera
 * Sleeper (out-reservation). King's Cross previously excluded LNER
 * (board-eligibility verdict `undecided` — this pack's own research
 * attempt could not obtain verifiable operator-policy text) but that was
 * resolved to `in` per the UK ledger, 5 Sep 2026 — King's Cross now passes
 * LNER through unfiltered (see jim-handoff.md for the prior undecided
 * history and docs/united-kingdom-ledger.md §3 for the resolution).
 *
 ...
 * for the two single-board exclusion groups (St Pancras
 * International/Eurostar, Paddington/Night Riviera Sleeper — King's
 * Cross/LNER no longer excludes, see the file-header note) and
