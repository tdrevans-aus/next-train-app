...
/**
 * Dedup key for `fetchStopBoard`'s `allTripsMap` — mode-agnostic (see
 * docs/jim-brief-dlr-dedup-key.md). TfL's Unified API `id`/`vehicleId` fields are trustworthy for
 * some modes (tube: measured unique, 37/37 at Oxford Circus) but not others (DLR's `/Arrivals`
 * feed reuses one `id`/`vehicleId` across every row at a stop, even across genuinely distinct
 * destinations — confirmed live at Abbey Road, 940GZZDLABR: 6 raw rows, 1 shared `id`, 3
 * destinations). Keying dedup on data that happens to be well-formed for one mode is exactly what
 * produced that bug, so this key does not rely *solely* on `arrival.id`/`vehicleId`.
 *
 * The brief's proposed key (line + resolved destination + departure time, platform "if it proves
 * necessary") was verified against the existing tube terminus fixture rather than taken on trust,
 * and it broke: two genuinely distinct Walthamstow Central-bound Victoria line trains in a real
 * 7 Sep 2026 capture share line, destination, platform *and* the same to-the-second departure time
 * (both were live, correctly-timed, and had different upstream ids — `-2066444382` and
 * `-411927803`), so the line/destination/platform/time-only key silently collapsed 29 real trips
 * to 28, a regression this fix must not introduce (acceptance criterion 4). So `id`/`vehicleId` is
 * folded back in as an extra field, not the sole key: it is the right shape to fix that case (two
 * real ids that differ) and provably can't reintroduce the DLR bug (DLR's rows already share the
 * *same* id/vehicleId, so adding a field that's constant across the colliding group can only ever
 * add discriminating power, never remove it — the destination/platform/time differences that
 * already separate Beckton/Woolwich Arsenal/Stratford International at Abbey Road keep doing so
 * whether or not `id` is also in the key).
 *
 * Uses fields that actually identify a *service*, plus the raw id as a tie-break rather than the
 * sole discriminator:
 *   - **collapses** the same train seen through the hub naptanId and its `alsoNaptanIds` fold-ins
 *     (docs/jim-brief-london-tram-duplicate-stops.md, PR #344) — the same physical arrival
 *     carries identical line/destination/platform/time/id across every naptanId it's fetched
 *     under (the `id` field identifies the vehicle prediction itself, not the queried stop, so it
 *     doesn't vary with which of the vehicle's stops you ask about — unlike DLR, where TfL fails
 *     to assign the id per-*vehicle* at all).
 *   - **preserves** genuinely distinct trains that happen to share an upstream `id`/`vehicleId`
 *     (DLR) — destination/platform/time still differ. It also now preserves genuinely distinct
 *     trains that happen to share line/destination/platform/time (the Walthamstow Central case
 *     above) — their differing `id` now differs the key too.
 */
 ...
 * plus every `alsoNaptanIds` fold-in) into a single deduped list keyed by `tripDedupKey`, keeping
 * the earlier `liveDeparture` on a genuine key collision. Exported (used internally by
