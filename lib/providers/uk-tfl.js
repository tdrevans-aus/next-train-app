...
function parseTflArrival(arrival, now = new Date()) {
  ...
  const destination = compassOrUnknown ? destinationName || towards || "Unknown" : towards;
  const lineName = String(arrival.lineName || arrival.lineId || "").trim();
  const fullDestination = lineName ? `${lineName} ${destination}`.trim() : destination;
...
export async function fetchStopBoard(stopIdOrName, options = {}) {
  ...
  // is fetched for the hub *and* every folded-in id so a board never loses
  // trains that only ever posted against a platform id. See
  // docs/jim-brief-london-tram-duplicate-stops.md.
  const naptanIdsToFetch = new Set();
  for (const entry of entries) {
    naptanIdsToFetch.add(entry.naptanId);
    for (const alsoId of entry.alsoNaptanIds ?? []) {
      naptanIdsToFetch.add(alsoId);
    }
  }

  const fetches = [...naptanIdsToFetch].map(async (naptanId) => {
    try {
      const arrivals = await tflFetch(`/StopPoint/${encodeURIComponent(naptanId)}/Arrivals`);
