...
export async function loadAmsterdamStatic() {
...
export async function fetchStationBoard(stationIdOrName, options = {}) {
  ...
  const trips = buildBoardForStops({
    ...
    cityId: "amsterdam",
    // OVapi's TripUpdates feed is nationwide (every Dutch operator); the
    // static snapshot here is trimmed to GVB metro only, so most
    // "unresolved" realtime trip IDs belong to other operators entirely,
    // not drift. Calendar coverage is still checked.
