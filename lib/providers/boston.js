 ...
 *
 * PR #431 review (Mark, 20 Sep 2026) found two further hard reds, both fixed here:
 *
 * 1. Every board row was duplicated exactly 2x. The old resolveStopIds() unioned a station's
 *    GTFS parent `place-*` id AND its platform child ids AND unrelated `door-*`/`node-*`
 *    pathway/entrance pseudo-stops into one `filter[stop]` query; MBTA's V3 predictions endpoint
 *    expands a parent-station id to also include its children's predictions, so querying the
 *    parent and its children together double-counts every real prediction. Confirmed live 20 Sep
 *    2026: `filter[stop]=place-miltt` alone returns 3 unique predictions; `filter[stop]=place-
 *    miltt,70267,70268` (parent + platforms together, the old query) returns 6 — the same 3
 *    trips, twice each. Fixed by querying only the single correct id per station (see item 2), and
 *    by defensively de-duplicating on the raw MBTA prediction `id` (never trip+stop+time, which a
 *    genuine same-minute same-trip re-prediction could otherwise still collide on) in
 *    mapPredictionsToTrips() below, so a future upstream quirk can't double a board again.
 * 2. Cold `/api/board` took 59-68s at South Station/Park Street because fetchStationBoard() still
 *    called loadGtfsStatic() (a full MBTA GTFS zip download + parse) on every cold start just to
 *    resolve a station name to an MBTA stop id — exactly the per-request static-GTFS cost this
 *    file's original fix was meant to remove. Fixed by resolving stop ids from
 *    lib/cities/boston/stations.json's bundled `mbtaStopIds` field instead (generated once,
 *    offline, by scripts/generate-boston-mbta-stop-ids.mjs — re-run it if MBTA restructures a
 *    station's ids). No GTFS zip is downloaded or parsed anywhere on the `/api/board`,
 *    `/api/next-train`, or `/api/directions` request path for Boston any more.
 *
 ...
 *   - a repeated prediction `id` (the same MBTA prediction resource appearing twice in one
 *     payload) — dropped after the first occurrence; belt-and-braces alongside the single-id
 *     query fix in item 1 above.
...
/** Kept only as a documented reference to where MBTA's GTFS static feed lives — the request
 * path (fetchStationBoard below) never fetches it. scripts/generate-boston-mbta-stop-ids.mjs is
 * the only thing that still downloads/parses this zip, offline, to (re)build stations.json's
 * `mbtaStopIds` field. */
...
/**
 * A catalog station's MBTA V3 stop id(s) — bundled data
 * (lib/cities/boston/stations.json's `mbtaStopIds`, generated offline by
 * scripts/generate-boston-mbta-stop-ids.mjs), never resolved from a live GTFS parse. Each entry
 * is exactly the id(s) that must be queried WITHOUT also querying a parent/child of the same
 * physical stop — the single GTFS parent (`place-*`) id for every catalog station as of 20 Sep
 * 2026 (confirmed live: querying a parent id together with its own platform child ids in the
 * same request double-counts every prediction, since MBTA's V3 API expands a parent to its
 * children — see file header item 1).
 */
export function resolveStopIds(catalogEntry) {
  return [...(catalogEntry.mbtaStopIds ?? [])];
}
...
/**
 * Maps a raw MBTA V3 predictions payload's `data` array to ProviderTrips, de-duplicating on the
 * prediction's own `id` first — belt-and-braces against a future upstream quirk re-sending the
 * same prediction resource twice in one payload (PR #431 review item 1), independent of the
 * single-stop-id-per-station query fix that is the primary defence against duplication.
 */
export function mapPredictionsToTrips(predictions, tripsById) {
  const seenPredictionIds = new Set();
  const trips = [];
  for (const prediction of predictions ?? []) {
    const predictionId = prediction?.id;
    if (predictionId != null) {
      if (seenPredictionIds.has(predictionId)) {
        continue;
      }
      seenPredictionIds.add(predictionId);
    }
    const trip = mapPredictionToTrip(prediction, tripsById);
    if (trip) {
      trips.push(trip);
    }
  }
...
export async function fetchStationBoard(stationIdOrName, options = {}) {
  ...
  const stopIds = options.stopIds ?? resolveStopIds(catalogEntry);
  if (!stopIds.length) {
    throw new Error(`Unknown Boston station: ${stationIdOrName}`);
  }
  ...
  const trips = mapPredictionsToTrips(payload?.data ?? [], tripsById).sort(
