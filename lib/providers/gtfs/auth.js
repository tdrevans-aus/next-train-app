...
/**
 * Swiftly Real-Time GTFS-RT (goswift.ly) — used by LA Metro Rail (`lametro-rail`), header
 * `Authorization: Bearer <key>`. Portal https://goswift.ly/realtime-api-key. Confirmed 29 Aug
 * 2026 (docs/los-angeles-d1/jim-handoff.md / published-network.json liveBoards.emptyKey): an
 * empty/missing key 401s on both vehicle-positions and trip-updates
 * (`WWW-Authenticate: Bearer`, body `{"errorCode":401,"errorMessage":"Invalid Credentials"}`) —
 * no real key was ever sent, none is pasted here. Not a D1 blocker; the city stays `planned`
 * regardless of whether this key is set.
 */
export function readSwiftlyApiKey() {
  return String(process.env.SWIFTLY_API_KEY || "").trim();
}

/**
 * Thrown whenever SWIFTLY_API_KEY is unset. Swiftly GTFS-RT (lametro-rail) is the ONLY
 * real-time board source for LA Metro Rail — Tim's rule: no live times, no board; never a
 * silent timetable fallback (same posture as lib/providers/bart.js / chicago.js /
 * washington.js). Every caller of lib/providers/los-angeles.js's fetchStationBoard must let
 * this propagate rather than catching it and returning an empty/synthetic board.
 */
export class MissingSwiftlyApiKeyError extends Error {
  constructor(message) {
    super(
      message ??
        "SWIFTLY_API_KEY is not set. LA Metro Rail live boards use Swiftly's GTFS-RT feed " +
          "(api.goswift.ly/real-time/lametro-rail/gtfs-rt-trip-updates) and there is no schedule " +
          "fallback — Tim's rule is no live times, no board. Register at " +
          "https://goswift.ly/realtime-api-key."
    );
    this.name = "MissingSwiftlyApiKeyError";
    this.envNames = ["SWIFTLY_API_KEY"];
  }
}

/** @param {string} [apiKey] */
export function swiftlyAuthHeaders(apiKey) {
  const trimmed = String(apiKey || "").trim();
  if (!trimmed) {
    throw new MissingSwiftlyApiKeyError();
  }
