...
function directoryCacheKey(directory, options = {}) {
  return JSON.stringify({
    ...
    sourceUrl: options.sourceUrl ?? directory,
  });
}
...
export function loadGtfsStaticFromDirectory(directory, options = {}) {
  ...
  const key = directoryCacheKey(directory, options);
  const cached = cacheByDirectoryKey.get(key);
  ...
  cacheByDirectoryKey.set(key, { data, loadedAt: Date.now() });
  return data;
}
