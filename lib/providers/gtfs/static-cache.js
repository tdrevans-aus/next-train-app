...
export async function loadGtfsStatic(options) {
  ...
  if (!response.ok) {
    throw new Error(`GTFS static download failed (${response.status}) for ${redactUrl(url)}`);
  }
...
/**
 * Date range (YYYYMMDD strings) a parsed snapshot's calendar.txt +
 * calendar_dates.txt cover — used by runtime staleness detection
 * (docs/jim-brief-gtfs-snapshot-freshness.md) to catch a snapshot that was
 * simply never refreshed (calendar ended in the past) or published wrong.
 */
export function snapshotCalendarRange(staticData) {
  let minDate = null;
  let maxDate = null;
  for (const row of staticData?.calendar ?? []) {
  ...
  for (const row of staticData?.calendarDates ?? []) {
    if (!row.date) {
      continue;
    }
...
/**
 * @returns {{ judgable: boolean, coversToday: boolean, todayYmd: string, minDate: string|null, maxDate: string|null }}
 *   judgable is false when the snapshot has no calendar/calendar_dates rows at
 *   all (nothing to compare against) — callers should not treat that as stale.
 */
export function snapshotCoversToday(staticData, now, timeZone) {
  const todayYmd = ymdInTimeZone(now, timeZone).replace(/-/g, "");
  const { minDate, maxDate } = snapshotCalendarRange(staticData);
  if (minDate === null && maxDate === null) {
    return { judgable: false, coversToday: true, todayYmd, minDate, maxDate };
  }
  const coversToday = (minDate === null || todayYmd >= minDate) && (maxDate === null || todayYmd <= maxDate);
  return { judgable: true, coversToday, todayYmd, minDate, maxDate };
}
