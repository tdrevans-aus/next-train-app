 ...
 *   putImpl is injectable so qa/gtfs-snapshot-freshness.mjs can round-trip
 *   the manifest without a live Blob token — @vercel/blob's `put` is a
 *   read-only ESM export and can't be monkeypatched from a test file.
 * @returns {Promise<{url:string}>}
 */
export async function writeManifest(city, manifest, { putImpl } = {}) {
  const put = putImpl ?? (await import("@vercel/blob")).put;
  const blob = await put(`gtfs/${city}.json`, JSON.stringify(manifest, null, 2), {
    access: "public",
    allowOverwrite: true,
