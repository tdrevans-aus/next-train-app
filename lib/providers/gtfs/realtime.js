...
export async function fetchTripUpdates(url, options = {}) {
  ...
  try {
    ...
    // A malformed/truncated upstream response (or an intermittent CDN hiccup) throws a raw
    // protobufjs error (e.g. "invalid group tag") whose message isn't safe or meaningful to
    // show a user. Treat it like an RT outage — board building already degrades gracefully to
    // schedule-only when realtimeIndex is empty — instead of letting the raw decode error
    // bubble up as a 500.
    console.warn(`GTFS-RT decode failed for ${url}: ${decodeError?.message ?? decodeError}`);
    entities = [];
  }
