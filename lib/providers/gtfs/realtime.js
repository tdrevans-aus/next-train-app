...
export function indexTripUpdates(entities, options = {}) {
  ...
  /**
   * Every trip_id the realtime feed referenced (post resolveTripId mapping),
   * whether or not it turned out to match anything in the static snapshot —
   * used by lib/providers/gtfs/board.js's runtime staleness check to compute
   * "share of realtime trip IDs that resolve against the snapshot"
   * (docs/jim-brief-gtfs-snapshot-freshness.md). Distinct from the maps
   * above, which only carry trips the feed had something to say about.
   */
  const tripIds = new Set();
  /**
   * Every trip_id the realtime feed itself carried, BEFORE resolveTripId's
   * mapping/filtering — several city adapters (gold-coast, newcastle,
   * canberra, amsterdam) pass a resolveTripId that returns null for anything
   * not already found in the static snapshot, which would otherwise make
   * `tripIds` above trivially 100%-resolved and blind to real drift. This is
   * the true denominator for "share of realtime trip IDs that resolve
   * against the snapshot".
   */
  const rawTripIds = new Set();
  ...
  for (const entity of entities) {
    ...
    const rawTripId = String(descriptor.tripId || "").trim();
    if (rawTripId) {
      rawTripIds.add(rawTripId);
    }
  ...
  return { tripDelaySec, stopUpdates, cancelledTrips, tripIds, rawTripIds };
}
