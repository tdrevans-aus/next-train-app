...
import { activeServicesForDate, snapshotCoversToday } from "./static-cache.js";
...
export function checkSnapshotFreshness({
  ...
  if (!skipResolvedShareCheck && share.judgable && share.share < STALE_RESOLVED_SHARE_THRESHOLD) {
    const reason = `only ${share.resolved}/${share.total} realtime trip IDs resolved against the snapshot (< ${Math.round(STALE_RESOLVED_SHARE_THRESHOLD * 100)}%)`;
