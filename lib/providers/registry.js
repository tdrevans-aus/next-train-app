...
export const CITIES = [
  ...
  },
  {
    ...
    integration:
      "One agency, one mode. National Rail reuses uk-darwin.js (Darwin OpenLDBWS) — same account-level blocker as every other UK region in this wave, DARWIN_LDB_TOKEN not set. Same 'no static GTFS at all' gap as West of England / London & South East National Rail — genuinely Darwin-or-nothing until the token exists. See lib/providers/southwest.js file header.",
    modes: ["train"],
    envKeys: ["DARWIN_LDB_TOKEN"],
    adapterReady: true,
    notes:
      "Region config over the shared uk-darwin.js provider (allow-list, no fork). Hub lock Exeter St Davids (EXD), secondary hub Plymouth (PLY), terminus Penzance (PNZ) — no doNotGroup needed at any of the three, single-layer GWR-dominated stations, platforms distinguished by route not operator. National Rail catalog: hub + secondary hub + terminus + through-running-only Taunton (TAU, West of England boundary — not a merge, matches West of England's own Taunton flag from the other side), Newton Abbot (NAB), Totnes (TON), Truro (TRU), St Austell (SAU), St Erth (SER). Night Riviera Sleeper excluded (out-reservation, compulsory sleeping-car cabin reservation) at the six stations it calls — Exeter St Davids, Plymouth, Truro, St Austell, St Erth, Penzance — enforced in code via excludeOperators, same mechanism as Rest of Scotland's Caledonian Sleeper exclusion, consistent with the same service's out-reservation verdict at Paddington in the London & South East National Rail pack. Direction model destination+operator (e.g. 'London Paddington (GWR)'), illustrative only — no destination strings confirmed until a live Darwin payload can be pulled. Open item for Tim only: OpenLDBWS/RDM Platform Agreement redistribution terms to third-party riders are unclear in public sources, same as every other UK region in this wave — do not relay Darwin data to end users until Tim confirms against the signed RDM Data Sharing Agreement. Europe/London, DST. D1 pack in docs/southwest-d1/. Do not flip live from this pack — Mark/Tim's call.",
  },
  {
    id: "cumbria",
    displayName: "Cumbria",
    ...
    integration:
      "One agency family, one mode. National Rail reuses uk-darwin.js (Darwin OpenLDBWS) — same account-level blocker as every other UK region in this wave, DARWIN_LDB_TOKEN not set. See lib/providers/cumbria.js file header.",
    modes: ["train"],
    envKeys: ["DARWIN_LDB_TOKEN"],
    adapterReady: true,
    notes:
      "Region config over the shared uk-darwin.js provider (allow-list, no fork) — the simplest UK station-graph shape this wave: ONE tier-1 hub lock (Carlisle, CAR — 8 platforms, junction of West Coast Main Line/Settle-Carlisle Line/Tyne Valley Line/Cumbrian Coast Line, plus Lakes Line reachable via Oxenholme) with TWO tier-2 secondary hubs (Oxenholme Lake District, OXO — WCML/Lakes Line branch point; Barrow-in-Furness, BIF — Furness Line/Cumbrian Coast Line junction), contrast Rest of Scotland's four co-equal hubs with no ranking basis. Penrith (PEN) is a major WCML station but not a junction — built regional, not promoted to hub. No doNotGroup case anywhere in this catalog — every junction is a single physical station, unlike Liverpool City Region's Lime Street or Greater Manchester's Piccadilly/Piccadilly Gardens. Regional/branch stations: Windermere (WND, Lakes Line terminus), Kendal (KND, Lakes Line), Settle (SLF, Settle-Carlisle Line southern boundary). Caledonian Sleeper carries board-eligibility verdict `out-reservation` (compulsory berth booking) per docs/board-eligibility-rule.md and is excluded from the board at Carlisle only — the sole Cumbrian station it calls — enforced in code via the shared uk-darwin.js excludeOperators option, same mechanism as Rest of Scotland's/Southwest's sleeper exclusions; unverified against a live payload since DARWIN_LDB_TOKEN is unset. Northern Trains/TransPennine Express/Avanti West Coast all verdict `in` everywhere they call; CrossCountry's Cumbrian calling pattern is unconfirmed and not added to any station's operator list. STATION COVERAGE GAP: only 7 of the report's claimed 48 Cumbrian stations are catalogued (its own aggregate count for the remainder is internally inconsistent, 38 vs. 41) — the other ~41 are a recorded D2 gap, not invented; needs a corrected/complete station list from Nico or a verified GTFS/Darwin pull. Regional boundaries checked against already-merged adjacent packs (Rest of Scotland, Greater Manchester) — no live overlap found at Lockerbie/Preston/Wigan/Settle today, remains an open D2 coordination point if either adjacent region's scope changes. Direction model destination+operator (e.g. 'Glasgow Central (Avanti West Coast)'), illustrative only — no destination strings confirmed until a live Darwin payload can be pulled. Open item for Tim only: OpenLDBWS/RDM Platform Agreement redistribution terms to third-party riders are unclear in public sources, same as every other UK region in this wave. Europe/London, DST. D1 pack in docs/cumbria-d1/. THIS IS THE LAST REGION IN THE CURRENT UK EXPANSION WAVE per the dispatch brief — confirm before dispatching further UK region packs whether the queue is genuinely exhausted. Do not flip live from this pack — Mark/Tim's call.",
  },
  {
    id: "south-wales",
  ...
  },
  {
    ...
    adapterReady: true,
    notes:
      "Region config over the shared uk-darwin.js provider (allow-list, no fork). TWO-HUB architecture, not a single hub-lock and not full flat multi-group (London SE Option A shape): Southampton Central (SOU) stands alone as the west-side hub; Portsmouth Harbour (PMH) is the east-side hub paired with Portsmouth & Southsea (PMS) as a secondary board on the same corridor/waterfront — reusing West of England's Bristol Temple Meads + Bath Spa hub+secondary-hub pattern, not promoting PMS to a third independent hub the way london-se-national-rail's seven termini are mutually independent. Southampton Central and the Portsmouth pair are independent of each other, never merged. No doNotGroup needed at any of the three boards — no report evidence of separate platform/boarding-section logic per operator, unlike London Bridge/Liverpool Street in london-se-national-rail. National Rail catalog: three stationGroups (SOU/PMH/PMS) + through-running junctions Fareham (FRM) and Eastleigh (ESL, no hub significance) + through-running-only boundary stations Westbury (WSB, West of England boundary) and Waterloo (WAT, London & South East National Rail boundary) — both boundary flags checked reciprocally consistent against those two regions' own already-merged packs, no de-dup action taken, D2/adapter-time concern for whichever region wires second. Island Line excluded entirely (out-mode, Wightlink FastCat ferry dependency to Ryde Pier Head from Portsmouth Harbour), not catalogued in any form. ADAPTER WIRED AND VERIFIED LIVE (5 Sep 2026, docs/solent-d1/jim-handoff.md 'Adapter wired' section) — lib/cities/solent/dogfood-next-train.js reuses the shared uk-darwin.js + uk/direction-hubs.js helpers (allow-list, no fork), dispatched from lib/cities/live-city-api.js's directionsFor()/getMultiCityNextTrain() switch-cases; not yet in MULTI_CITY_IDS (list membership is bundled into the flip commit, per CLAUDE.md's flip-follow-through split). All seven CRS codes confirmed live against Darwin 5 Sep 2026 — crsVerified: true throughout (Fareham FAR->FRM was the one wrong code, corrected in stations.json/published-network.json/this pack's prose before adapter wiring; see qa/solent-dogfood-gate.mjs's catalog CRS sweep). Direction hub anchoring: ONE hub wired, at Fareham (FRM) only — 'Portsmouth Harbour' (filterCrs PMH), collapsing an operator-split duplicate (Great Western Railway + South Western Railway both print 'Portsmouth Harbour' at Fareham, Liverpool-Lime-Street shape). Southampton Central shows the IDENTICAL operator split at Fareham (Southern + South Western Railway) but has NO configured hub there — the shared lib/cities/uk/direction-hubs.js helper's findHubForStation() supports only one hub per appliesFrom station, and editing that shared helper was out of scope for this pack (guardrail); still routes correctly via the exact-chip path, just shown as two operator-suffixed chips rather than one collapsed label. Flagged for Tim/Luke as a shared-helper follow-up if this multi-hub-per-station shape recurs elsewhere. Eastleigh (ESL) was probed for the same operator-split shape and for the brief's Kidderminster-shape candidate (west-of-Southampton termini absorbed into a Southampton Central hub) — both rejected on live filtered-board evidence (see direction-hubs.json notes and jim-handoff.md). Direction model destination+operator, confirmed live against a real Darwin payload 5 Sep 2026. Open item for Tim only: OpenLDBWS/RDM Platform Agreement redistribution terms to third-party riders are unclear in public sources, same as every other UK region in this wave — do not relay Darwin data to end users until Tim confirms against the signed RDM Data Sharing Agreement. Also open for Tim only, not resolved here: (1) confirm the two-hub/hub+secondary architecture is the intended shape — the oracle report explicitly asked for Tim/Luke/Jim review before this pack proceeded; (2) Portsmouth & Southsea's real ridership weight relative to Portsmouth Harbour, in case it warrants promotion to a fully independent hub; (3) the Fareham single-hub-per-station shared-helper limitation above. Europe/London, DST. D1 pack in docs/solent-d1/. STATUS STAYS PLANNED — do not flip live from this pack — Mark/Tim's call. Flip commit — list additions for Mark (per CLAUDE.md's flip-follow-through split; dispatch switch-cases, dogfood module, and qa/solent-dogfood-gate.mjs are already wired ahead of the flip and need no further change): add 'solent' to (1) lib/cities/live-city-api.js's MULTI_CITY_IDS array and MultiCityId typedef, (2) brisbane-dogfood.js's mount/available map, (3) journey-model.js's persisted-city/country lists — bundle these three list-membership edits into the status-flip commit itself, not before, per qa/live-city-lists-sync.mjs.",
  },
  {
    id: "thames-valley",
