...
export const CITIES = [
  ...
  },
  {
    ...
    adapterReady: true,
    notes:
      "Separate city from Auckland. TRAIN only (Kāpiti, Hutt Valley, Melling, Johnsonville, Wairarapa). No bus/ferry/cable car. D1 pack in docs/wellington-d1/. MEL live terminus Western Hutt while Melling Station closed. DST. Flipped live by Tim, 30 Aug 2026. GTFS-RT fixed 30 Aug 2026: Metlink's API gateway does exact string matching on Accept, not real content negotiation — the shared realtime.js multi-type Accept default (works fine for AT/TfNSW) wasn't recognised, so Metlink silently served JSON instead of protobuf, which the protobuf decoder then failed to parse (\"invalid wire type 6\"). wellington.js now sends a bare \"application/x-protobuf\" Accept header to get real protobuf back; live delay/cancellation data now flows.",
  },
  {
    id: "amsterdam",
  ...
  },
  {
    ...
    integration:
      "Trafiklab GTFS Regional skane. Static (TRAFIKLAB_API_KEY) verified 200 2026-08-30. GTFS-RT TripUpdates needs a separate Trafiklab-issued key (TRAFIKLAB_API_KEY_RT) — Static and Realtime are billed/scoped as distinct products, confirmed 403 'Key does not have access to file' when using the Static key against TripUpdates.pb 2026-08-30. VehiclePositions/ServiceAlerts catalogued but not wired.",
    modes: ["train"],
    envKeys: ["TRAFIKLAB_API_KEY", "TRAFIKLAB_API_KEY_RT"],
    adapterReady: true,
    notes:
      "Pågatågen only — 10 corridors, 84 unique stations. No bus, no tram (closed 1973), no Öresundståg, no Krösatågen. No passenger-facing line codes anywhere; chips are product + far end (Pågatågen mot <X>). Hub lock Malmö C — also the network's one double-call station (Malmöringen/line 11, Oslo-Stortinget pattern). doNotGroup Pågatågen vs Öresundståg at Malmö C/Triangeln/Hyllie/Burlöv. DST. D1 pack in docs/malmo-d1/. Static key scope confirmed working 2026-08-30; TRAFIKLAB_API_KEY_RT still needed for live TripUpdates before flip. Do not flip live.",
  },
  {
    id: "uppsala",
    displayName: "Uppsala",
    timeZone: "Europe/Stockholm",
    status: "planned",
    agency: "Mälartåg (regional rail v1 cut); ticketing authority Upplands Lokaltrafik (UL)",
    integration:
      "Trafiklab GTFS Regional ul. Static (TRAFIKLAB_API_KEY) verified 200 2026-08-30, and ul.zip DOES embed Mälartåg trips — confirmed 2026-08-30 by downloading the live bundle: agency 'Mälardalstrafik' with 3 route_type-100 railway routes (Arlanda C/Märsta, Gävle, Sala), separate from UL's own all-bus agency. GTFS-RT TripUpdates needs a separate Trafiklab-issued key (TRAFIKLAB_API_KEY_RT) — confirmed 403 'Key does not have access to file' against ul's TripUpdates.pb using the Static key 2026-08-30; whether Mälartåg trips specifically appear in that RT feed (vs. only UL buses) is still unconfirmed pending that second key.",
    modes: ["train"],
    envKeys: ["TRAFIKLAB_API_KEY", "TRAFIKLAB_API_KEY_RT"],
    adapterReady: true,
    notes:
      "Mälartåg regional rail only — four corridors through Uppsala C (Gävle, Sala/Västerås/Eskilstuna, Stockholm-via-Märsta, Stockholm/Örebro-via-Arlanda). No UL buses (route_type 700 only, city vs regional bus not splittable from GTFS), no SJ, no Snälltåget, no SL Line 40 (that's Stockholm's own line, already modeled in docs/stockholm-d1/published-network.json). No passenger-facing line codes — chips are product + far end (Mälartåg mot <X>). Hub lock Uppsala C — a clean spoke terminus, no double-call hazard unlike Malmö C. Knivsta-fork headsign ambiguity (Stockholm C via Märsta vs via Arlanda) open for Tim, see direction-model-memo.md. DST. D1 pack in docs/uppsala-d1/. Static feed content confirmed 2026-08-30 (Mälartåg trips present); TRAFIKLAB_API_KEY_RT still needed to confirm live TripUpdates before flip. Do not flip live.",
  },
  {
    id: "osaka",
    displayName: "Osaka",
