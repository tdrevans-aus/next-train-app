...
export const CITIES = [
  ...
  },
  {
    ...
    status: "live",
    agency: "Metro Trains / V/Line",
    integration:
      "GTFS Schedule + GTFS-RT static-join, Transport Victoria Open Data Portal (header KeyID); Metro Trains (folder 2) + V/Line walk-up services (folder 1) merged",
    modes: ["train"],
    envKeys: ["VIC_OPENDATA_API_KEY"],
    adapterReady: true,
    notes:
      "22 Sep 2026: replaced the PTV Timetable API path (devid/PTV_API_KEY key never arrived) with the Open Data Portal's GTFS-RT feeds — live-verified across all 16 currently-running Metro line groups (Racecourse is special-events-only, not running at verification time) and all 13 V/Line route_ids at the shared stations. See docs/melbourne-d1/jim-handoff.md. Direction labels relabelled to terminus-only/Perth style the same day (docs/jim-brief-melbourne-direction-labels-perth-style.md) — bare terminus except at the shared hub Flinders Street, which carries the line name in parentheses.",
  },
  {
    id: "brisbane",
  ...
  },
  {
    ...
    agency: "STIB/MIVB",
    integration:
      "LIVE via the BMC Waiting Times API (confirmed 20 Sep 2026): `GET https://api-management-opendata-production.azure-api.net/api/datasets/stibmivb/rt/WaitingTimes`, header Ocp-Apim-Subscription-Key. The operation path was previously unconfirmed (guessed against the client-rendered developer-portal SPA) — recovered from the portal's own public `/mapi/apis?api-version=2018-06-01-preview` content-listing endpoint, which enumerates every published API's real path. Verified live with a real key at Arts-Loi / Kunst-Wet (all four v1 lines) and Simonis/Elisabeth (lines 2 and 6), including a live-only 'Do not embark' operational message that cannot exist in a static schedule — confirmation this is genuine vehicle tracking, not schedule echo. No numeric rate limit was exposed in response headers; treated conservatively (15s cache TTL). Catalog `stopIds` ARE STIB point IDs (no separate id-mapping step needed). No timetable fallback: MissingStibCredentialsError / StibUnavailableError are hard errors — see lib/providers/brussels.js file header. SECOND SOURCE (20 Sep 2026, docs/jim-brief-brussels-flip-readiness.md): SNCB/NMBS domestic rail at the three shared stations named below via iRail liveboard (https://api.irail.be/liveboard/, no key, lib/providers/irail.js) — a genuinely separate provider surface layered on top, not a fork of the STIB pipeline. iRail failing degrades that station's board to metro-only + `partial: true`, it never refuses the board (STIB failing still refuses, unchanged).",
    modes: ["metro", "train"],
    envKeys: ["STIB_API_KEY"],
    adapterReady: true,
    notes:
      "Metro 1/2/5/6 only (60 unique stops). No tram, no premetro/North-South Axis, no CHRONO, no bus, no De Lijn, no TEC. No passenger metro 3 or 4 — Albert-Bordet is a frozen project. Hub lock Arts-Loi / Kunst-Wet (metro 1x2x5x6, through-cross: 1/5 E-W trunk, 2/6 inner-ring loop N-S) — never a direction token. Simonis and Elisabeth are two distinct line-2/6 terminus chips, never collapsed. Bilingual FR/NL locked names with a slash (Stockel / Stokkel, Gare du Midi / Zuidstation, ...); same-in-both-languages names (Beekkant, Simonis, Elisabeth, Schuman, ...) stay single. DST. D1 pack in docs/brussels-d1/. STIB_API_KEY is in Vercel (Production/Preview/Development). Flip follow-through landed 20 Sep 2026 (docs/brussels-d1/jim-handoff.md): lib/cities/brussels/dogfood-next-train.js + qa/brussels-dogfood-gate.mjs (replacing brussels-planned-gate.mjs) + live-city-api.js dispatch switch-cases; deliberately NOT added to MULTI_CITY_IDS/brisbane-dogfood.js/journey-model.js yet — that's Mark's flip commit. STIB metro live board CONFIRMED AND WIRED 20 Sep 2026 (see integration field) — this is no longer a flip blocker. SNCB/NMBS BOARD-ELIGIBILITY GAP CLOSED 20 Sep 2026 (docs/jim-brief-brussels-flip-readiness.md): docs/brussels-d1/oracle-clash-report.md's Board eligibility section rules SNCB/NMBS domestic rail `in` at Gare Centrale / Centraal, Gare du Midi / Zuidstation, and Gare de l'Ouest / Weststation — now wired as a second live source (iRail liveboard, see integration field), doNotGroup-by-mode (`mode`/`agency` fields on every trip), Eurostar/Thalys/TGV INOUI/OUIGO/Nightjet/European Sleeper filtered out by vehicle type (ICE stays `in`, optional reservation outside a short summer peak). Belgium/Brussels picker + CITY_BOUNDS + flip-commit edit list also landed this pass — see docs/brussels-d1/jim-handoff.md \"Flip commit — exact edits\". S-TRAIN LIVE-CAPTURE BUG FIXED 20 Sep 2026 (Mark's PR #419 QA, docs/brussels-d1/board-eligibility-addendum.md): live iRail always sends S-trains as `S` + a sub-line number (S1/S2/S3/S8/S10, never a bare `S`) — classifySncbVehicleType() now matches by pattern instead of an exact `\"S\"` string, closing a silent-drop bug that hid every S-train at all three shared stations. EC (EuroCity) and ECD (EuroCity Direct) also ruled `in` per the addendum (no compulsory reservation) and added to the classifier; rail-replacement BUS rows classify `out-mode`, not unmapped. Any iRail vehicle type this module doesn't recognise is now surfaced via mapIrailDepartures()'s `unmapped` property / the board's `debug.irailUnmappedTypes` field, and qa/brussels-dogfood-gate.mjs fails if the captured-live fixture produces any — so a future silent drop is detectable, not invisible. qa/fixtures/brussels/irail-liveboard.json is now a real 20 Sep 2026 iRail capture at Brussels-Central/Brussels-South (S10/S1/S8/S3/EC/IC/S2/ECD verbatim from the live payload) plus hand-authored rows for out-reservation kinds not running at capture time. Live-verified post-fix: Gare de l'Ouest / Weststation now shows real S10 SNCB departures (was 0 rail rows before this fix); Gare du Midi shows a real ECD row; Gare Centrale shows a real EC row. Do not flip live from this pack — Mark/Tim's call.",
  },
  {
    id: "copenhagen",
  ...
  },
  {
    ...
    integration:
      "MBTA V3 predictions (https://api-v3.mbta.com/predictions, unauthenticated, confirmed working 20 Sep 2026, rate-limited ~20/min) via lib/providers/boston.js — a standalone adapter (own agency, not a shared/regional provider). Subway/rapid transit AND Commuter Rail are BOTH built from live V3 predictions, no static-schedule fallback anywhere on the board (fixed 20 Sep 2026, docs/jim-brief-boston-subway-live-predictions.md — subway used to be static-GTFS schedule-only with realtime:false, which shipped scheduled times as live and was also catastrophically slow). One predictions call per station (filter[route_type]=0,1,2) covers a dual-mode station's subway and Commuter Rail rows together; a short-TTL, in-flight-coalescing, stale-on-error cache (same shape as lib/providers/brussels.js's fetchWaitingTimesRaw) means one upstream call serves every direction a station's board fans out to, keeping normal traffic inside the unauthenticated rate limit. MBTA static GTFS (https://cdn.mbta.com/MBTA_GTFS.zip, lib/providers/gtfs/static-cache.js loadGtfsStatic) is used ONLY to resolve a station name/alias to MBTA's own stop_ids — structural reference data, never a source of a departure time. Stop-name resolution is exact-match (lib/providers/boston.js resolveStopIds) against lib/cities/boston/stations.json's per-station alias list — deliberately not gtfs/static-cache.js's findRailStopIdsForName, whose substring match would collapse documented same-name-family pairs (Harvard vs Harvard Ave, Central vs Central Ave, etc.). Subway trips are classified by exact V3 route_id (Red/Orange/Blue/Green-B/C/D/E/Mattapan). A prediction is dropped (never becomes a board row) when its schedule_relationship is CANCELLED/SKIPPED, its revenue is NON_REVENUE, or its departure_time is null (arrival-only/terminating) — never backfilled from a schedule. Any MBTA V3 fetch failure (down/rate-limited) with no usable stale cache entry propagates as a thrown error — the board refuses, same posture as lib/providers/bart.js/chicago.js/washington.js, never a scheduled fallback. MBTA_API_KEY is an optional env var (raises the rate limit to ~1000/min); no key is registered by this task, though one is advisable before production traffic. MissingMbtaApiKeyError is exported for a future caller that wants to make a key mandatory.",
    modes: ["metro", "train"],
    adapterReady: true,
    notes:
      "Subway/rapid transit (live MBTA V3 predictions, no scheduled fallback — fixed 20 Sep 2026, docs/jim-brief-boston-subway-live-predictions.md): Red (Alewife-Ashmont & Braintree), Orange (Oak Grove-Forest Hills), Blue (Wonderland-Bowdoin), Green B/C/D/E, Mattapan (Ashmont-Mattapan, its own line, not folded into Red) — 125 unique stops, 8 passenger services. PLUS MBTA Commuter Rail (docs/boston-d1/oracle-clash-report.md Board eligibility section, 20 Sep 2026 — walk-up on-board/kiosk ticketing, no check-in barrier) at the five stations it calls in-catalog: South Station, North Station, Forest Hills, Braintree, JFK/UMass — live V3 predictions only, own terminus-only chips, line name added only at the two shared hubs (e.g. 'Worcester', 'South Station (Framingham/Worcester Line)' — relabelled 22 Sep 2026, docs/jim-brief-direction-label-aliases-server-side.md) since direction-model-memo.md has no Commuter Rail guidance (flagged for Tim). Amtrak (Acela, Northeast Regional, Lake Shore Limited) at South Station is `out-reservation` — all-reserved, no walk-up; the Commuter Rail route allow-list only ever contains MBTA's own CR route ids, so Amtrak trips can't appear. CapeFlyer is board-eligibility `in` (seasonal, walk-up) but is NOT served by api-v3.mbta.com — no CapeFlyer route exists in the V3 routes list as of 20 Sep 2026, so it can't be shown live; this is a feed gap, not a decision to exclude it. Silver Line BRT, ferry, bus, Massport shuttles stay `out-mode`. Hub lock Park Street (Red x Green, all four Green services) — never a direction token; subway direction model is 'line + terminus' (e.g. 'Red Line + Alewife', 'Green Line B + Gov't Center') per direction-model-memo.md §3 recommendation A, which resolves headsigns only to each line's own known termini so a hub string can never leak into a direction chip. doNotGroup (hazard-pack.md H1/H2/H6): Downtown Crossing (Red x Orange), Gov't Center (Green x Blue; B/C inner end terminus — legitimate as a direction, forbidden only as a Park Street proxy), State (Orange x Blue), South Station, North Station, Haymarket — all separate stop-places from Park Street; Commuter Rail is a distinct service entry at the same physical stop-place at the five dual-mode stations, not folded into the subway rows. Red forks at JFK/UMass (Ashmont vs Braintree). Green B/C end at Gov't Center; D continues to Union Sq; E continues to Medford/Tufts and leaves the B/C/D trunk at Copley (no Hynes/Kenmore). America/New_York HAS DST (EDT/EST) — do not copy Perth/Brisbane no-DST. No stopIds baked into the catalog (none in the D1 pack) — stop_ids are resolved at request time by exact name match against the static feed (structural reference only, never a time source). Flip follow-through done (dogfood module, live-city-api.js dispatch, boston-dogfood-gate.mjs). Flipped live 20 Sep 2026 (docs/boston-d1/mark-qa-note.md third pass) — MULTI_CITY_IDS/journey-model persisted lists/brisbane-dogfood mount map/picker/CITY_BOUNDS all updated in the same commit per qa/live-city-lists-sync.mjs. Subway moved from static-GTFS schedule-only to live V3 predictions the same day (docs/jim-brief-boston-subway-live-predictions.md — the schedule-only board had shipped scheduled times as `realtime:false` but still presented as a live board, plus a ~30-60s cold board due to per-direction fan-out each doing its own GTFS parse). D1 pack in docs/boston-d1/.",
  },
  {
    id: "chicago",
    displayName: "Chicago",
  ...
  },
  {
    ...
    integration:
      "WMATA Station Prediction API (https://api.wmata.com/StationPrediction.svc/json/GetPrediction, docs https://developer.wmata.com/) via lib/providers/washington.js — a standalone adapter (own agency, not a shared/regional provider and not a fork of the GTFS helpers, since Station Prediction is a pure real-time board; WMATA's own GTFS/GTFS-RT are portal-keyed too, no free fallback). LIVE BOARDS ONLY, per Tim's rule: no live times, no board; never a silent timetable fallback — same posture as lib/providers/bart.js and lib/providers/chicago.js, deliberately no GTFS-static schedule-only degrade path. fetchStationBoard() throws MissingWmataApiKeyError (lib/providers/gtfs/auth.js) whenever WMATA_API_KEY is unset, and propagates any WMATA fetch/parse failure rather than returning an empty/synthetic board. WMATA_API_KEY was confirmed live against both endpoints (docs/washington-d1/jim-handoff.md, re-confirmed fresh 25 Sep 2026 per docs/washington-d1/mark-qa-note.md) — station StationCodes are resolved at request time from WMATA's own Rail Station Information endpoint (jStations) by name match, rather than a hand-transcribed code table. WashingtonStationCodeUnconfirmedError is thrown rather than guessing when the jStations join can't find a qualifying candidate. Min values 'ARR'/'BRD' are treated as an imminent (0-minute) departure and kept; '---'/empty/non-numeric Min values are dropped before ever reaching a rider; Line values 'No' (no-passenger) and '--' (unknown) are dropped.",
    modes: ["metro"],
    envKeys: ["WMATA_API_KEY"],
    adapterReady: true,
    notes:
      "WMATA Metrorail only — six colour lines (Red Shady Grove-Glenmont, Orange Vienna/Fairfax-GMU-New Carrollton, Blue Franconia-Springfield-Downtown Largo, Silver Ashburn-Downtown Largo & New Carrollton, Green Greenbelt-Branch Av, Yellow Huntington-Mt Vernon Sq/7th St-Convention Center & Greenbelt) — 98 unique passenger stops. No Metrobus, no Streetcar, no DC Circulator. Hub lock Metro Center (Red × Orange/Blue/Silver transfer, a through-cross not a single-end hub — Red is NW-NE, Orange/Blue/Silver are E-W) — never a direction token; ONE catalog station, both platform codes (A01+C01) queried together at request time via WMATA's own StationTogether1/2 linkage, never split into two catalog entries or hardcoded. Direction model is 'Line + terminus' (e.g. 'Red Line + Glenmont', 'Silver Line + Ashburn', 'Yellow Line + Huntington'), per direction-model-memo.md §3 recommendation A — keeps the word 'Line', unlike Chicago's bare colour word. doNotGroup/doNotCollapse (hazard-pack.md H1/H4/H6, lib/cities/washington/marketing-directions.js file header): Metro Center vs Gallery Place-Chinatown (one stop apart, different transfers), Farragut North (Red only) vs Farragut West (Orange/Blue/Silver only) — two separate stations, two separate streets. No same-printed-name-different-place families exist on this map (hazard-pack.md H5) — unlike Chicago/BART, resolveCatalogEntry() never throws an ambiguous-name error here. Board eligibility (oracle-clash-report.md, controller note 20 Sep 2026): MARC and VRE pass both walk-up tests at shared stations (Union Station, Rockville, Silver Spring, New Carrollton, L'Enfant Plaza, Franconia-Springfield) but come from feeds other than WMATA's API — ruled `out-product` by Tim, 20 Sep 2026: a separate agency real-time feed and provider relationship is not justified before launch; revisit post-launch. NOT wired in this adapter, not faked; no longer holds the flip. All Amtrak services, Northeast Regional included, are 'out-reservation'. Station coordinates (lib/cities/washington/stations.json) are sourced from DC GIS's public 'Metro Stations Regional' ArcGIS FeatureServer layer (WMATA-sourced open data, no key required), matched by name/alias — NOT its embedded TRAININFO_URL station-code fragments, which have at least one known data bug (Potomac Yard duplicates Huntington's C15); WMATA StationCodes are always resolved live from jStations instead, never from that dataset. America/New_York HAS DST (EDT/EST) — do not copy Perth/Brisbane no-DST. City id is washington; do not invent city=dc/washington-dc/wmata/us, do not merge into chicago or bart. Flipped live 25 Sep 2026 (docs/washington-d1/mark-qa-note.md, fully green) — MULTI_CITY_IDS/journey-model persisted lists/brisbane-dogfood mount map all updated in the same commit per qa/live-city-lists-sync.mjs; qa/washington-dogfood-gate.mjs rewritten to assert live state (docs/jim-brief-washington-live-gate.md), mirroring qa/boston-dogfood-gate.mjs's post-flip shape. D1 pack in docs/washington-d1/.",
  },
  {
    id: "los-angeles",
    ...
    timeZone: "America/Los_Angeles",
    status: "planned",
    agency: "LA Metro (Los Angeles County Metropolitan Transportation Authority)",
    integration:
      "Swiftly GTFS-RT (`lametro-rail`, https://api.goswift.ly/real-time/lametro-rail/gtfs-rt-trip-updates, header `Authorization: Bearer <SWIFTLY_API_KEY>`, portal https://goswift.ly/realtime-api-key) via lib/providers/los-angeles.js — reuses the shared GTFS static/RT helpers (lib/providers/gtfs/static-cache.js, realtime.js, board.js), same pattern as lib/providers/auckland.js, since Swiftly serves genuine GTFS-RT (unlike BART/Chicago/WMATA's bespoke prediction APIs). LA Metro's own static rail GTFS (https://gitlab.com/LACMTA/gtfs_rail/raw/master/gtfs_rail.zip, no key) is used ONLY at request time to resolve a catalog station name to GTFS stop_id(s) (findRailStopIdsForName) — never a departure-time source, and explicitly NOT the generator for lib/cities/los-angeles/stations.json (hand-transcribed from the official printed map per the D1 pack). LIVE BOARDS ONLY, per Tim's rule: no live times, no board; never a silent timetable fallback — same posture as lib/providers/bart.js/chicago.js/washington.js. fetchStationBoard() throws MissingSwiftlyApiKeyError (lib/providers/gtfs/auth.js) whenever SWIFTLY_API_KEY is unset (confirmed empty-key 401 on both Swiftly endpoints, 29 Aug 2026, docs/los-angeles-d1/jim-handoff.md), and propagates any Swiftly fetch/parse failure rather than returning an empty/synthetic board. **SWIFTLY_API_KEY is required before any live board — Tim must register at https://goswift.ly/realtime-api-key.** Route-id-to-line mapping (LA_METRO_ROUTE_SHORT_NAME_TO_LINE) and resolveTerminus() destination strings are UNVERIFIED against a real payload; confirm both once a key is set.",
    modes: ["metro", "light_rail"],
    envKeys: ["SWIFTLY_API_KEY"],
    adapterReady: true,
    notes:
      "LA Metro Rail only — six passenger lines: A (Pomona North-Downtown Long Beach, clockwise Long Beach loop via 5th St/1st St/Pacific Av), B (North Hollywood-Union Station), C (Norwalk-LAX/Metro Transit Center), D (Union Station-Wilshire/La Cienega, further west under construction and out of v1), E (Downtown Santa Monica-Atlantic), K (Expo/Crenshaw-Redondo Beach) — 110 unique open passenger stops (Metro Facts prints 115 system-wide; line-tick count matches). No G Line or J Line Busway, no Metro bus, no Metrolink, no Amtrak, no LAX FlyAway/airport people movers. Hub lock 7th St/Metro Ctr (A x B x D x E, a through-cross, not a single-end hub — A runs N-S through it, E runs E-W through it, B/D continue on to Union Station/North Hollywood/Wilshire/La Cienega) — never a direction token; C and K never call here. Direction model is 'line + terminus' (e.g. 'A Line + Pomona North', 'E Line + Atlantic', 'K Line + Redondo Beach'), per direction-model-memo.md §3 recommendation A — keeps the word 'Line', same shape as Washington, unlike Chicago's/BART's bare colour word. A Line's documented short-turn destinations (Monrovia, APU/Citrus College, Wardlow) are recorded (A_LINE_SHORT_TURNS) but never treated as a terminus chip. doNotGroup/doNotCollapse (hazard-pack.md H1/H4/H6, lib/cities/los-angeles/marketing-directions.js file header): 7th St/Metro Ctr vs Union Station vs Civic Ctr/Grand Park vs Pershing Square vs Historic Broadway vs East LA Civic Ctr (six distinct downtown/transfer buildings, no same-printed-name-different-place collisions exist on this map otherwise, hazard-pack.md H5); Grand Av Arts/Bunker Hill vs Grand/LATTC vs LATTC/Ortho Institute; Pico vs Pico/Aliso; Crenshaw vs Expo/Crenshaw; Aviation/Century vs Aviation/Imperial; Pacific Av (A Line Long Beach loop stop, not a second south terminus) vs Downtown Long Beach. Board eligibility (hazard-pack.md H3, coverageGaps): Metrolink/Amtrak at Union Station and Pomona North, and LAX FlyAway/airport people movers at LAX/Metro Transit Center, are separate agencies/buildings — not in-catalog stations, doNotGroup; none wired here, none faked. America/Los_Angeles HAS DST (PDT/PST) — do not copy Perth/Brisbane no-DST. City id is los-angeles; do not invent city=la/lax/metro/lacmta/us, do not merge into washington/chicago/bart/boston. D1 pack in docs/los-angeles-d1/. Do not flip live from this pack — Mark/Tim's call.",
  },
  {
    id: "dublin",
    displayName: "Dublin",
    ...
    status: "planned",
    agency: "Luas (Keolis / National Transport Authority)",
    integration:
      "GTFS static-join over the National Transport Authority's (Ireland) GTFS-RT v2 feed, via lib/providers/dublin.js — reuses the shared gtfs/ helpers (loadGtfsStatic, buildBoardForStops, ovapi-tripupdates-cache.js's generic TripUpdates cache, same as lib/providers/melbourne.js/adelaide.js), not a fork. Static: NTA's national GTFS_All.zip (https://www.transportforireland.ie/transitData/Data/GTFS_All.zip, ~160MB, verified 200 no key) covers every Irish operator — too large to fetch on the request path (same reasoning as Melbourne's statewide zip-of-zips and the retired Amsterdam/Rotterdam OVapi refresh, docs/jim-brief-newcastle-stale-snapshot.md), so the adapter reads gtfsFixtureBlobUrl('dublin') instead; scripts/trim-dublin-gtfs.mjs trims the national zip to Luas-only routes (route_short_name/route_long_name matching 'luas'/'red'/'green', UNVERIFIED against a live payload — Luas prints no route number, hazard-pack.md H2 — confirm and tighten to agency_id at D2) but was NOT run/published this session (no BLOB_READ_WRITE_TOKEN available) — gtfsFixtureBlobUrl('dublin') will 404 until published; this is a D2/pre-flip task, not a D1 blocker. Realtime: NTA GTFS-RT v2 TripUpdates (https://api.nationaltransport.ie/gtfsr/v2/TripUpdates, header x-api-key, confirmed 404 without a key by the D1 pack) — NTA_API_KEY was NOT available this session (not in .env.local); MissingNtaApiKeyError (lib/providers/gtfs/auth.js) propagates rather than a silent timetable fallback, live-only same as lib/providers/melbourne.js (a trip without RT confirmation is dropped, never shown as live from the schedule alone — docs/jim-brief-boston-subway-live-predictions.md).",
    modes: ["metro"],
    envKeys: ["NTA_API_KEY"],
    adapterReady: true,
    notes:
      "Luas Red + Green light rail ONLY (docs/dublin-d1/jim-handoff.md v1 scope) — no DART/Iarnród Éireann (commuter rail, v2), no Dublin Bus, no Bus Éireann, no Go-Ahead Ireland, no premetro. 67 unique stops total: Red 32 (two southwestern branches forking at Belgard — Tallaght via Cookstown/Hospital, Saggart via Fettercairn/Cheeverstown/Citywest Campus/Fortunestown — sharing one common trunk east to The Point) plus Green 35 (Broombridge to Brides Glen, including a one-way city-centre loop). No printed route numbers — colour-only branding (Red #E2231A, Green #39B54A); gtfsRouteIdsIfKnown left empty in the D1 pack for D2 verification against a real NTA payload, not invented. Hub lock Abbey Street (Red trunk stop between Jervis and Busáras, confirmed by dot position on the official Luas Network Map) — chosen over Connolly (Red only, DART interchange, no Green access) per the D1 pack's own hub-lock recommendation; never a direction token. Green interchange (Marlborough / O'Connell - GPO / O'Connell Upper) is a ~200 m walk, not a shared platform — Abbey Street boards must never show a Green chip. SHARPEST HAZARD (hazard-pack.md H4a): the Green Line's Parnell<->Trinity city-centre loop is direction-exclusive, not a simple branch — O'Connell - GPO and O'Connell Upper are served northbound-only (towards Broombridge); Marlborough is served southbound-only (towards Brides Glen); Trinity and Parnell are the two merge points and see both directions. lib/cities/dublin/marketing-directions.js's isDirectionAllowedAtStop() drops a trip whose classified direction doesn't match what one of those three stops can genuinely serve, rather than risk showing a phantom platform/direction that never runs — flagged in the D1 pack as needing confirmation against a second source (live NTA GTFS-RT trip patterns) once keyed, direction-model-memo.md open question 5. Direction model is colour + terminus (e.g. 'Red + Tallaght', 'Red + Saggart', 'Green + Broombridge', 'Green + Brides Glen'), per direction-model-memo.md §3 recommendation A — Red's two branches are only distinguishable by terminus, there is no line number to fall back on. doNotGroup (hazard-pack.md H1): Abbey Street vs Marlborough/O'Connell - GPO/O'Connell Upper (hub vs walkable Green cluster); O'Connell - GPO vs O'Connell Upper (two distinct Green stops, same name family, direction-exclusive); Tallaght vs Saggart (two distinct Red termini forking at Belgard); Red Cow vs Kingswood vs Belgard (three consecutive distinct stops — Belgard, not Red Cow, is the actual fork). No stopIds baked into the catalog (none in the D1 pack) — stop_ids are resolved at request time by name match against the (not-yet-published) static feed, same runtime-resolution pattern as lib/providers/malmo.js/copenhagen.js. Connolly's map dogleg is treated as an inline stop per the D1 pack's general-knowledge assumption, flagged for a second-source confirmation (hazard-pack.md H4, direction-model-memo.md open question 6) — not resolved here. Europe/Dublin — do NOT hand-roll a fixed-offset/'no DST' rule (hazard-pack.md H7: the oracle report's 'no daylight saving observed since 2024' claim looks wrong/unverified; Ireland's clock-change abolition proposal stalled at EU level and was never enacted) — IANA tzdata handles the real, current transition rule. City id is dublin; do not invent city=dub/ie, do not merge into a national multi-city Irish feed. D1 pack in docs/dublin-d1/. Do not flip live from this pack — Mark/Tim's call.",
  },
];
