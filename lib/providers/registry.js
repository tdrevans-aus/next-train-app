...
export const CITIES = [
  ...
  },
  {
    ...
    adapterReady: true,
    notes:
      "GVB metro M50–M54 only. Not city=nl. Hub Centraal Station (not NS Amsterdam Centraal). DST. Testers live; not a store listing. OVapi GTFS-RT live (re-enabled 5 Sep 2026, PR #261); static fixture refreshed manually (see codebase-inventory 6.7). Shared 20 s TripUpdates cache, 5 s timeout, live / timetable marker (PR #261).",
  },
  {
    id: "rotterdam",
    ...
    adapterReady: true,
    notes:
      "RET metro A–E only. Separate from amsterdam. Not city=nl or the-hague. Hub Beurs. DST. Testers live; not a store listing. OVapi GTFS-RT live (re-enabled 5 Sep 2026, PR #261); static fixture refreshed manually (see codebase-inventory 6.7). Shared 20 s TripUpdates cache, 5 s timeout, live / timetable marker (PR #261).",
  },
  {
    id: "vancouver",
  ...
  },
  {
    ...
    status: "planned",
    agency: "Wiener Linien GmbH",
    integration:
      "Wiener Linien OGD static GTFS (https://www.wienerlinien.at/ogd_realtime/doku/ogd/gtfs/gtfs.zip, no key) via lib/providers/vienna.js — a standalone adapter (own agency, not a shared/regional provider), reusing lib/providers/gtfs/static-cache.js (loadGtfsStatic) and lib/providers/gtfs/board.js (buildBoardForStops) rather than forking them. Filtered to GTFS route_type 1 (subway/metro) plus an exact route_id allow-list (U1, U2, U3, U4, U6). Real-time OGD Realtime Monitor (https://www.wienerlinien.at/ogd_realtime/monitor, no key) is a proprietary JSON schema, NOT GTFS-RT protobuf, and is NOT wired at D2 — MissingViennaRealtimeMonitorError documents the unwired live path. Schedule-only board.",
    modes: ["metro"],
    adapterReady: true,
    notes:
      "U-Bahn heavy metro only: U1 (Leopoldau-Oberlaa, 24), U2 (Karlsplatz-Seestadt, 21), U3 (Ottakring-Simmering, 21), U4 (Hütteldorf-Heiligenstadt, 20), U6 (Floridsdorf-Siebenhirten, 24) — 99 unique stations (corrects the oracle report's claimed 28/20/20/19/22 per-line and ~109 total; see docs/vienna-d1/hazard-pack.md H2). U5 excluded (under construction, service opens 2030). No S-Bahn, no Badner Bahn (Wiener Lokalbahnen), no ÖBB national rail, no tram, no bus — all out-product/out-mode per the oracle report's Board eligibility section. Hub lock Karlsplatz (U1 x U2 x U4) — U1/U4 through-stations, U2 terminates there (its own printed southern terminus, a different hub shape from every prior city's pure through-cross hub) — never a direction token. Direction model 'line + terminus' (e.g. 'U1 + Leopoldau'); U2's LINE_TERMINI deliberately excludes Karlsplatz so the only live U2 chip anywhere is 'U2 + Seestadt' (direction-model-memo.md section 2 — flagged for Tim to confirm before D5 assertion tables). doNotGroup the nine two-line interchanges (Praterstern, Stephansplatz, Volkstheater, Schottenring, Schwedenplatz, Landstraße, Westbahnhof, Längenfeldgasse, Spittelau) against the Karlsplatz hub identity. Kaisermühlen-VIC locked as the fuller station-page-confirmed name (alias: truncated 'Kaisermühlen' from the alphabetical master table). Two flagged, non-blocking D1 items carried forward, not resolved here (docs/expansion-tracker/pack-readiness-2026-09-06.md): (1) the oracle report's claimed 'Schedifkaplatz' (U6 x Badner Bahn interchange) is unverified against any primary source pulled for the pack — Badner Bahn is out-product regardless, so this doesn't change v1 scope; (2) no nested short-turn codes were found at D1, but this was checked against Wikipedia prose only, not GTFS trip patterns — flag for a D2 timetable pass. Route classification (VIENNA_ROUTE_TYPES + exact route_id allow-list) is unverified against a live GTFS payload, same caveat class as Copenhagen/Boston. License CC BY 4.0 (Vienna OGD portal), attribution required. Europe/Vienna HAS DST. No Austria country-lane ledger (single-region country). D1 pack in docs/vienna-d1/. Do not flip live from this pack — Mark/Tim's call.",
  },
];
