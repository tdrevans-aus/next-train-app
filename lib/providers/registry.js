...
export const CITIES = [
  ...
  },
  {
    ...
    integration:
      "Two agencies. National Rail via uk-darwin.js (Darwin OpenLDBWS, DARWIN_LDB_TOKEN live 2 Sep 2026). Merseyrail static GTFS is confirmed live (Transitland f-gc-rail~delivery~group~planar~gtfs, verified 2026-08-31), but real-time is genuinely UNCONFIRMED — no public GTFS-RT endpoint found anywhere, only an undocumented mobile-app internal API — same kind of gap as Greater Manchester's Metrolink case and South Yorkshire's Supertram/SYFTL case. fetchMerseyrailStopBoard() throws MerseyrailFeedUnconfirmedError (known permanent gap, not swallowed) per board-eligibility rule and Tim's approval (2 Sep 2026). See lib/providers/liverpool-city-region.js file header.",
    modes: ["train", "metro"],
    envKeys: ["DARWIN_LDB_TOKEN"],
    adapterReady: true,
    notes:
      "Region config over the shared uk-darwin.js provider (allow-list + direction model) plus a Merseyrail metro-mode catalog, same pattern as south-yorkshire/east-midlands/north-east/greater-manchester — not a fork. V1 scope is National Rail (live) + Merseyrail catalogued but out-product (board eligibility corrected 2 Sep 2026 at flip time — see below) — a deliberate deviation from the oracle report's own headline recommendation to defer Merseyrail to H2, flagged as open item 1 in docs/liverpool-city-region-d1/direction-model-memo.md; justified as precedented with Metrolink and Supertram, which share the identical unconditional-throw shape. BOARD ELIGIBILITY CORRECTION (2 Sep 2026): the oracle report originally verdicted Merseyrail `in` on the strength of 'schedule-only' language, but no schedule-only board was ever implemented here (or in Metrolink/Supertram) — fetchMerseyrailStopBoard() throws MerseyrailFeedUnconfirmedError unconditionally, same as those siblings' own unconfirmed-feed errors, both of which correctly verdict `out-product`. Corrected to match: Merseyrail is `out-product`, not `in` — see docs/liverpool-city-region-d1/oracle-clash-report.md's Board eligibility section. TWO STRUCTURALLY SEPARATE AGENCY SHAPES: National Rail hub Liverpool Lime Street (LIV CRS, seven TOCs), secondary hub Liverpool South Parkway (LPY CRS). Merseyrail interchange pair Liverpool Central / Moorfields (both named 'dual-line interchange' by report, no ranking given; neither built as sole hub). LIME STREET STRUCTURAL AMBIGUITY (H1) UNRESOLVED, CARRIED AS-IS — report contradicts itself on whether Merseyrail at Lime Street shares platforms ('platform level', report line 41) or separate infrastructure (line 73, C2/C3 point 2), no distance/time figure given. Kept as built: TWO SEPARATE stationGroups (mode train vs mode metro, same printed name), doNotGroup: true, conservative separate-infrastructure reading — not resolved by this flip; see docs/liverpool-city-region-d1/hazard-pack.md H1. Merseyrail: Northern Line (Liverpool-Southport/Ormskirk/Headbolt Lane) + Wirral Line (Liverpool-Ellesmere Port/West Kirby/Chester), line+terminus model, termini/branches-only (Lime Street Central/Moorfields/Ellesmere Port explicitly named; full 69-stop order is a D2 gap). Marketing chips generated only for genuinely confirmed termini (Ellesmere Port); Liverpool Central/Moorfields unconfirmed as destination call points, not chipped. ELLESMERE PORT REGISTRY: Tim deleted `uk-ellesmere-port` as a hangover concept (never shipped). This region's own Ellesmere Port entry (genuine Wirral Line terminus per report) stands as the sole reference; no merge absorption needed. Open items for Tim only: (1) Lime Street/Merseyrail physical relationship confirmation (walk-distance figure, site documentation, Darwin response); (2) Merseyrail real-time feed status (contact data@merseyrail.org); (3) OpenLDBWS/RDM redistribution terms (same as every UK region); (4) Liverpool Central vs Moorfields ranking, if exists. docs/united-kingdom-ledger.md still absent — flagged per country-lane retrofit, overdue. Europe/London, DST. D1 pack in docs/liverpool-city-region-d1/. Live as of 2 Sep 2026.",
  },
  {
    id: "greater-anglia",
