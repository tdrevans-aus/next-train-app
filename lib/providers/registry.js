...
export const CITIES = [
  ...
  },
  {
    ...
    integration:
      "Trafiklab GTFS Regional ul. Static (TRAFIKLAB_API_KEY) verified 200 2026-08-30, and ul.zip DOES embed Mälartåg trips — confirmed 2026-08-30 by downloading the live bundle: agency 'Mälardalstrafik' with 3 route_type-100 railway routes (Arlanda C/Märsta, Gävle, Sala), separate from UL's own all-bus agency. GTFS-RT TripUpdates needs a separate Trafiklab-issued key (TRAFIKLAB_API_KEY_RT) — confirmed 403 'Key does not have access to file' against ul's TripUpdates.pb using the Static key 2026-08-30; Mälartåg trips confirmed live 2026-08-30 via decoded realtime feed.",
    modes: ["train"],
    envKeys: ["TRAFIKLAB_API_KEY", "TRAFIKLAB_API_KEY_RT"],
    adapterReady: true,
    notes:
      "Mälartåg regional rail only — four corridors through Uppsala C (Gävle, Sala/Västerås/Eskilstuna, Stockholm-via-Märsta, Stockholm/Örebro-via-Arlanda). No UL buses (route_type 700 only), no SJ, no Snälltåget, no SL Line 40 (that's Stockholm's own line). Hub lock Uppsala C — a clean spoke terminus, no double-call hazard. DST. D1 pack in docs/uppsala-d1/. Static feed content confirmed 2026-08-30; live Mälartåg TripUpdates verified 2026-08-30 (7 trips on sample board). Flipped live by Tim, 30 Aug 2026.",
  },
  {
    id: "osaka",
    displayName: "Osaka",
