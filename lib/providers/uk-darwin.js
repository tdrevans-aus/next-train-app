...
/**
 * Server-side cache in front of Darwin, per docs/uk-architecture.md and
 * docs/uk-provider-design.md — every UK region shares this one cache since
 * they all go through fetchDepartureBoard(). Boundary sits at the raw-JSON
 * layer (before parseDepartureBoardJson/providerTripToInternal and before
 * any excludeOperators/includeOperators filtering in fetchStationBoard), so
 * a filtered and an unfiltered caller for the same CRS+request-shape share
 * one upstream call.
 *
 * Key: CRS + numRows + filterCrs + filterType (the request-shaping options
 * that actually reach the upstream fetch) — not operator filters, which are
 * applied by callers after this layer.
 */
export const DARWIN_CACHE_TTL_MS = 20_000;
const DARWIN_CACHE_STALE_MAX_MS = 60_000;

/** @type {Map<string, { value: any, timestamp: number, inflight: Promise<any>|null }>} */
const darwinBoardCache = new Map();

function darwinCacheKey({ crs, filterCrs, filterType, numRows }) {
  return JSON.stringify({
    crs: String(crs || "").toUpperCase(),
    filterCrs: filterCrs ? String(filterCrs).toUpperCase() : "",
    filterType: filterCrs ? filterType || "to" : "",
    numRows: Number(numRows) || 10,
  });
}

function evictExpiredDarwinCacheEntries(now) {
  for (const [key, entry] of darwinBoardCache) {
    if (!entry.inflight && now - entry.timestamp > DARWIN_CACHE_STALE_MAX_MS) {
      darwinBoardCache.delete(key);
    }
  }
}
...
async function fetchDepartureBoardUncached({ crs, filterCrs, filterType = "to", numRows = 10 }) {
 ...
 * Cached 20s (DARWIN_CACHE_TTL_MS), with in-flight coalescing and
 * stale-on-error (up to 60s) — see the comment block above.
 * @param {{ crs: string, filterCrs?: string, filterType?: "to"|"from", numRows?: number, noCache?: boolean }} params
 */
export async function fetchDepartureBoard(params) {
  if (params?.noCache) {
    return fetchDepartureBoardUncached(params);
  }

  const key = darwinCacheKey(params);
  const now = Date.now();
  const existing = darwinBoardCache.get(key);

  if (existing?.inflight) {
    return existing.inflight;
  }
  if (existing && now - existing.timestamp < DARWIN_CACHE_TTL_MS) {
    return existing.value;
  }

  const inflight = fetchDepartureBoardUncached(params).then(
    (value) => {
      darwinBoardCache.set(key, { value, timestamp: Date.now(), inflight: null });
      evictExpiredDarwinCacheEntries(Date.now());
      return value;
    },
    (error) => {
      const stale = darwinBoardCache.get(key);
      if (stale && Date.now() - stale.timestamp < DARWIN_CACHE_STALE_MAX_MS) {
        darwinBoardCache.set(key, { ...stale, inflight: null });
        if (!loggedStaleOnErrorOnce) {
          loggedStaleOnErrorOnce = true;
          console.error(
            `Darwin fetch failed, serving stale cache entry for ${key}: ${error?.message ?? error}`
          );
        }
        return stale.value;
      }
