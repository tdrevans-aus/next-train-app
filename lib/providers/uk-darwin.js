...
const catalogPath = join(__dirname, "../cities/uk/stations.json");
...
function resolveCatalogEntry(stationIdOrName) {
  ...
  for (const entry of stationCatalog.stations ?? []) {
    if (normalizeKey(entry.name) === needle || normalizeKey(entry.crs) === needle) {
      return entry;
    }
  ...
  if (/^[a-z]{3}$/i.test(needle)) {
    return { name: needle.toUpperCase(), crs: needle.toUpperCase() };
  }
...
export function listCatalogStations() {
  return (stationCatalog.stations ?? []).map((entry) => ({
    name: entry.name,
    crs: entry.crs,
