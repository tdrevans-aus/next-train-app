 ...
 *
 * Static: a Vercel Blob snapshot (gtfsFixtureBlobUrl("goteborg")) published from
 * Trafiklab GTFS Regional `vt` (TRAFIKLAB_API_KEY) by
 * scripts/publish-gtfs-snapshot-to-blob.mjs — never fetched from Trafiklab directly on
 * the request path, which 429'd under normal cold-start traffic (see
 * docs/jim-brief-sweden-static-429-and-key-leak.md). Trafiklab still publishes no TripUpdates for `vt` (availability table blank
...
export function goteborgLineCode(trip) {
  ...
  // `mapVasttrafikDeparture` sets `transportMode` explicitly. The GTFS
  ...
  // only), so `trip.transportMode` is undefined/empty there and every gate
  // below passes unconditionally.
  ...
  const board = await fetchGtfsRealtimeBoard(stationIdOrName, {
    ...
    mapTrip(trip) {
      ...
      return {
        ...
        transportMode: transportModeForCode(code || trip.routeShortName),
...
function isAllowedLiveTrip(trip) {
  if (trip.destination === null) {
    // See fetchTimetableBoard's matching filter: no chip this station offers
    // matches this trip (through-running past the terminus).
    return false;
  }
