...
export async function fetchStationBoard(stationIdOrName, options = {}) {
  ...
  const trips = buildBoardForStops({
    ...
    // Same OVapi nationwide-feed-vs-trimmed-snapshot mismatch as Amsterdam
    // (docs/jim-brief-gtfs-snapshot-freshness.md) — see amsterdam.js's
    // equivalent comment. Not given a cityId (rotterdam is out of this
    // brief's scope per the 7 Sep release-1 scope cut) so no staleness
    // registry recording, but calendar coverage is still checked.
