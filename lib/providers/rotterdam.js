...
const FIXTURE_DIR = join(ROOT, "qa/fixtures/rotterdam/gtfs");
...
async function loadRotterdamStatic() {
  if (existsSync(join(FIXTURE_DIR, "stops.txt"))) {
    return loadGtfsStaticFromDirectory(FIXTURE_DIR, {
      ...
      sourceUrl: ROTTERDAM_GTFS_STATIC_URL,
    });
  }
  return loadGtfsStatic({
    url: ROTTERDAM_GTFS_STATIC_URL,
    routeTypes: ["1"],
    includeRouteShortNames: ROTTERDAM_METRO_SHORT_NAMES,
    agencyIds: ["RET"],
    timeZone: ROTTERDAM_TIME_ZONE,
...
export async function fetchStationBoard(stationIdOrName, options = {}) {
  ...
  try {
    ...
    const realtime = await fetchTripUpdates(ROTTERDAM_GTFS_RT_TRIP_UPDATES_URL, {
      headers: ovapiHeaders(),
      timeoutMs: 2500,
    });
