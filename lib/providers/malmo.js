 ...
 *   `loadStatic` defaults to `loadMalmoStatic` (the Vercel Blob snapshot); QA passes an
 *   offline/synthetic override so the closure-board path can be exercised without a network
 *   fetch (docs/jim-brief-malmo-planned-closure-empty-board.md, round 3).
 */
export async function fetchStationBoard(stationIdOrName, options = {}) {
  ...
  const board = await fetchGtfsRealtimeBoard(stationIdOrName, {
    loadStatic: options.loadStatic ?? loadMalmoStatic,
    resolveStopIds,
    resolveCatalogEntry,
    tripUpdatesUrl,
    timeZone: MALMO_TIME_ZONE,
