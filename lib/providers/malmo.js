 ...
 *
 * Static (TRAFIKLAB_API_KEY): https://opendata.samtrafiken.se/gtfs/skane/skane.zip?key=…
 ...
 *
 * v1 scope: Pågatågen regional rail only (route_long_name prefix "Pågatåg", which also covers
 * PågatågenExpress). No bus, no Öresundståg (separate cross-border product, same stations at
 * Malmö C/Triangeln/Hyllie/Burlöv — doNotGroup), no Krösatågen, no light rail.
 */
...
function tripAllowed(trip) {
  return PAGATAG_PREFIX.test(String(trip.routeLongName || ""));
}

async function loadMalmoStatic() {
  const key = requireTrafiklabApiKey();
  const url = trafiklabGtfsStaticUrl(MALMO_OPERATOR, key);
  ...
  const board = await fetchGtfsRealtimeBoard(stationIdOrName, {
    ...
    mapTrip(trip) {
      ...
      return {
        ...trip,
        lineId,
        destination: mapMalmoDestination(trip.destination, lineId, stationName),
      };
    },
  });
...
export { MALMO_HUB, resolveCatalogEntry };
