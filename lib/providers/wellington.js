...
function authHeaders() {
  return {
    ...metlinkAuthHeaders(readMetlinkApiKey()),
    // Metlink's API gateway does simple string matching on Accept, not real content
    // negotiation: the shared realtime.js default ("application/x-google-protobuf,
    // application/x-protobuf, application/octet-stream" — a multi-type list that AT and
    // TfNSW both parse fine) doesn't match anything Metlink recognises, so it silently
    // falls back to serving the feed as JSON. Feeding it JSON bytes through the protobuf
    // decoder is what produced "invalid wire type 6" on every request. A bare single-value
    // "application/x-protobuf" is the only Accept value Metlink actually honours with a
    // real 2.0 GTFS-RT FeedMessage (confirmed via manual byte inspection 2026-08-30).
