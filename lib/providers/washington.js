 ...
 * file. WMATA_API_KEY was NOT available this session (not in .env.local) — this adapter was
 * never called against the live endpoint; station codes are resolved by name match against
 * jStations rather than a hand-transcribed code table specifically to avoid fabricating codes
 * with no key/network access this session (documented station codes, e.g. from the public DC
 * GIS "Metro Stations Regional" dataset used to source lib/cities/washington/stations.json's
 * lat/lng, are NOT trusted for StationCodes — that dataset has at least one known data-entry bug,
 * Potomac Yard's TRAININFO_URL fragment duplicating Huntington's C15 — so codes are always
 * resolved live from WMATA's own jStations, never from that dataset).
 *
 * `Min` field handling (documented, UNVERIFIED against a live payload):
 *   - "ARR" (arriving) and "BRD" (boarding) are treated as an imminent departure (0 minutes) —
 *     both are real live predictions, kept.
 ...
 *     returns an empty trip list, not a fabricated-looking live row.
 *   - A positive integer string is minutes-until-arrival, added to `now` (Date arithmetic is
 *     timezone-agnostic; DST is resolved correctly at *display* time via Intl formatting in
 *     WASHINGTON_TIME_ZONE, hazard-pack.md H7 — America/New_York HAS DST).
 * `Line` field handling: "No" (no-passenger/out-of-service train) and "--" (unknown route) are
 * DROPPED — no-passenger trains must never appear on a rider-facing board. Any other value not
 * in WMATA_LINE_CODE_TO_LINE (lib/cities/washington/marketing-directions.js) is also dropped,
 * belt-and-braces against an unexpected route token ever reaching a rider.
 *
 ...
 * registry.js notes and jim-handoff.md addendum). Amtrak rows are `out-reservation` (all-reserved,
 * no walk-up) except Northeast Regional, which is `in` but likewise not wired (same feed gap).
 */
 ...
 * tests — never call the live endpoint from a QA gate; this pack's shape is documented-only,
 * unverified live. */
