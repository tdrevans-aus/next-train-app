 ...
 *
 * Board eligibility (docs/bart-d1/oracle-clash-report.md "Board eligibility" section, controller
 * note 20 Sep 2026): Caltrain (Millbrae) and Capitol Corridor/Amtrak (Richmond, Oakland Coliseum)
 * both pass the walk-up tests but are ruled `out-product` by Tim, 20 Sep 2026 — v1 ships on
 * BART's own ETD API only; each needs a separate agency feed (511.org / Amtrak, the latter's
 * availability and terms unverified per docs/united-states-ledger.md) not justified before
 * launch; revisit post-launch. Muni Metro and VTA Light Rail are `out-mode`. None of these are
 * wired in this adapter and must not be faked.
 *
