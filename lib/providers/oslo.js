...
/**
 * Board horizon (docs/jim-brief-oslo-board-horizon.md, 20 Sep 2026): a fixed
 * `numberOfDepartures` split across every line/direction at a stop starved busy hubs like
 * Stortinget (10 directions, 1-2 trains each). Entur's `estimatedCalls` supports
 * `numberOfDeparturesPerLineAndDestinationDisplay`, which caps departures per line+direction
 * combo independently rather than splitting one shared pool — confirmed live against Entur's
 * schema 20 Sep 2026 (introspection: `numberOfDeparturesPerLineAndDestinationDisplay: Int`,
 * "Limit the number of departures per line and destination display returned. The parameter is
 * only applied when the value is between 1 and 'numberOfDepartures'"; `timeRange: Int`,
 * default 24h). `numberOfDepartures` is kept as a generous overall safety cap (well above
 * combos-at-a-stop × per-line cap, confirmed live at Stortinget: 12 T-bane combos × 8 = 96 of
 * a 200 cap) so the per-line limit is what actually governs, not the total. `timeRange`
 * doesn't need widening past Entur's 24h default to reach 45 minutes, but is set explicitly
 * to 90 min anyway so a low-frequency Vy/Flytoget direction is bounded to a sane forward
 * window rather than depending on the (much larger) implicit default. Live-verified 20 Sep
 * 2026 at
 * Stortinget/Jernbanetorget/Nationaltheatret/Majorstuen/Vestli: every T-bane direction reaches
 * ≥6 departures / 76-90 min ahead; Vy/Flytoget directions reach at least their true frequency
 * within the window (e.g. VY L2->Ski genuinely runs only once in 90 min — not starved, just
 * infrequent) without crowding out low-frequency lines.
 */
const DEFAULT_NUMBER_OF_DEPARTURES = 200;
...
function buildStopPlaceQuery(stopPlaceId, options) {
  ...
  return {
    query: `query OsloStationBoard($id: String!, $n: Int!, $perLine: Int!, $timeRange: Int!) {
      stopPlace(id: $id) {
        id
        name
        estimatedCalls(
...
export async function fetchStationBoard(stationIdOrName, options = {}) {
  ...
  const stopPlaces = await Promise.all(
    stopPlaceIds.map((id) => fetchEnturStopPlace(id, enturOptions))
  );
  ...
  const rawCalls = stopPlaces.filter(Boolean).flatMap((stopPlace) => stopPlace.estimatedCalls ?? []);
  const trips = estimatedCallsToTrips(rawCalls, now);
 ...
 * (classify -> map -> drop cancelled/past -> sort) against a fixture without mocking `fetch`.
 * Does not filter on `call.realtime`: `estimatedCalls` already blends scheduled and
 * realtime-confirmed calls, and this adapter has never filtered non-realtime calls out of the
 * board (each trip carries its own `realtime` boolean) — unchanged by the horizon fix
 * (docs/jim-brief-oslo-board-horizon.md).
 */
export function estimatedCallsToTrips(rawCalls, now = new Date()) {
