...
export async function loadGoldCoastStatic() {
...
export async function fetchStationBoard(stationIdOrName, options = {}) {
  ...
  const trips = buildBoardForStops({
    ...
    // rail, tram); the static snapshot here is trimmed to G:link only, so
    // most "unresolved" realtime trip IDs are out-of-scope buses/trains,
