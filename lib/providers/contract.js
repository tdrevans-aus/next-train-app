 ...
 * @property {boolean} [realtime] per-trip freshness marker (docs/jim-brief-brussels-scheduled-tail-and-sncb-grouping.md):
 *   `true` when this row comes from a live/tracked source (set today by Brussels' STIB metro
 *   rows and SNCB/iRail rows — lib/providers/brussels.js, lib/providers/irail.js); `false` when
 *   it is a timetable-only row appended after a city's live rows run out (Brussels' scheduled
 *   metro tail, lib/providers/brussels.js). Omitted/undefined for every provider that hasn't
 *   adopted the marker yet — that is NOT the same as `false` and must not be treated as
 *   scheduled-only. When `false`, `enrichTripTiming()` (lib/train-times-core.js) forces
 *   `status` to the literal string `"Scheduled"` regardless of any scheduled/display time
 *   diff, and `buildTripPayload()` copies `realtime` through to the `/api/board` and
 *   `/api/next-train` trip JSON unchanged, so the marker survives to the client even though no
 *   UI reads the raw field directly (the UI reads `status === "Scheduled"` and the
 *   `.scheduled` CSS class it drives — public/app.js, public/styles/hero.css). The standing
 *   rule this implements: scheduled times are never presented as live, and are never shown
 *   when live data is unavailable (see docs/feature-backlog.md FB-57 and
 *   lib/providers/brussels.js's scheduled-tail comment for the "only while the live feed is
 *   healthy" enforcement).
 */
