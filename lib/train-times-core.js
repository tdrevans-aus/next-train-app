...
/**
 * jim-brief-brussels-scheduled-tail-and-sncb-grouping: a trip explicitly marked
 * `realtime: false` (contract.js) is a timetable-only row — always shown as
 * "Scheduled", never run through the on-time/delayed diff (which would be
 * meaningless for a row whose live and scheduled times are, by construction,
 * identical). `realtime: true` or `undefined` (every existing provider) is
 * unaffected.
 */
export function enrichTripTiming(trip, apiDisplayDelay = "") {
  ...
  return {
    ...
    status:
      trip.realtime === false
        ? "Scheduled"
        : resolveTripDisplayStatus(timingOffsetMinutes, apiDisplayDelay),
  };
}
