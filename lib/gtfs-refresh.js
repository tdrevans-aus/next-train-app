...
export const STANDALONE = [
  // canberra: a 403 here whose body is a Cloudflare "Just a moment..." page
  // (or a `cf-mitigated: challenge` response header) does NOT mean the URL
  // or User-Agent is wrong - it means this function is not running in the
  // syd1 region. Cloudflare in front of transport.act.gov.au serves a
  // managed JS challenge to this project's default US egress (iad1) but
  // returns 200 from syd1. The fix is a per-function region pin in
  // vercel.json's functions["api/health.js"] entry, not anything in this
  // file. See docs/jim-brief-canberra-refresh-region.md (13 Sep 2026).
...
export async function downloadZip(url, extraHeaders = {}) {
  ...
  if (!response.ok) {
    // Surface Cloudflare's `cf-mitigated: challenge` response header (seen for
    // Canberra's upstream when this function runs outside syd1 - see the
    // STANDALONE comment above and docs/jim-brief-canberra-refresh-region.md)
    // in the thrown message, so the status record explains itself without a
    // log dive. The `download failed (<status>)` prefix is left unchanged -
    // qa/gtfs-refresh-partial-status-gate.mjs may match on it.
    const cfMitigated = response.headers.get("cf-mitigated");
    const suffix = cfMitigated ? `, cf-mitigated=${cfMitigated}` : "";
    throw new Error(`download failed (${response.status}${suffix}) for ${url}`);
  }
