...
console.log(
  `north-east-dogfood-gate: ok (live, in MULTI_CITY_IDS, dispatch switch-cases wired, D1 pack, ${region.railCount} rail + ${region.metroCount} Metro stations, doNotGroup at Brockley Whins/East Boldon/Heworth/Manors/Seaburn (UK station fill phase 2b) and Newcastle Central via distinct printed names, Sunderland correctly modelled as NOT doNotGroup (still Metro-feed-blocked), Pelaw confirmed Metro-only and served by both lines, National Rail directions derived live from Darwin with no static line map, exact-chip routing table (exact/undirected) proven token-free with the national rail-crs-index fallback for out-of-region termini, Metro dispatch correctly surfaces MetroFeedUnconfirmedError rather than the static label list, all ${region.metroCount} Metro stops flagged liveFeed: false and excluded from the picker/Near me, Newcastle (Australia) and Perth (Australia) stay green)`
);
