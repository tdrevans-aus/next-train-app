/**
 * D6 — Live Gold Coast G:link sweep. Not a CI gate.
 *   npm run sweep:gold-coast
 */
...
import {
  GOLD_COAST_TIME_ZONE,
  fetchStationBoard,
  listCatalogStations,
} from "../lib/providers/gold-coast.js";
import { marketingLabelsForStation } from "../lib/cities/gold-coast/marketing-directions.js";
...
async function main() {
  ...
  if (live?.ok !== true) {
    throw new Error("assertCityLive(gold-coast) must pass");
  }
  ...
  for (const station of stations) {
    ...
    if ((board.trips ?? []).some((trip) => /varsity|beenleigh|ferry|bus /i.test(trip.destination))) {
      anomalies.push({ station: station.name, check: "C1", detail: "board leaked SEQ train/bus/ferry" });
    }
  }

  mkdirSync(join(ROOT, "qa/reports"), { recursive: true });
  const timestamp = new Date().toISOString().replace(/[:.]/g, "-");
  const reportPath = join(ROOT, "qa/reports", `gold-coast-sweep-${timestamp}.json`);
  writeFileSync(reportPath, `${JSON.stringify({ timeZone: GOLD_COAST_TIME_ZONE, anomalies, results }, null, 2)}\n`);
  console.log(`gold-coast-network-sweep: ${anomalies.length} anomalies → ${reportPath}`);
}
