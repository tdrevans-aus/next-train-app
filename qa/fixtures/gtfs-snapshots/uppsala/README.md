# uppsala — local calendar-only GTFS fixture
...
**Source zip:** last-known-good `uppsala.zip` preserved 10 Sep 2026 during the
`next-train-gtfs` Blob suspension (docs/jim-brief-blob-transfer-reduction.md), fetched before the
suspension from `gtfsFixtureBlobUrl("uppsala")`
...
```bash
node scripts/extract-gtfs-calendar-fixture.mjs uppsala <path-to-fresh-zip>
```
