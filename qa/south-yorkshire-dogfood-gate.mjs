...
console.log(
  `south-yorkshire-dogfood-gate: ok (live, in MULTI_CITY_IDS, dispatch switch-cases wired, D1 pack, ${region.railCount} rail + ${region.metroCount} Supertram stations, doNotGroup at Sheffield Station and Meadowhall Interchange/Meadowhall (through-running switch point, not a second hub lock), National Rail directions derived live from Darwin with no static line map, exact-chip routing table (exact/undirected) proven token-free with the national rail-crs-index fallback for out-of-region termini, Supertram dispatch correctly surfaces SupertramFeedUnconfirmedError rather than the static label list, all ${region.metroCount} Supertram stops flagged liveFeed: false and excluded from the picker/Near me, Perth Australia stays green)`
);
