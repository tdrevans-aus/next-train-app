/**
 * Greater Manchester adapter/dispatch wiring gate + flip-commit assertions.
 * Wired by Jim (PR #277: dogfood module, dispatch switch-cases), flipped by Mark
 * (this commit: status live, list additions). Per CLAUDE.md's flip-follow-through
 * split (added 30 Aug 2026, corrected same day): status flip, MULTI_CITY_IDS
 * addition to live-city-api.js / brisbane-dogfood.js / journey-model.js / app.js /
 * city-session.js, LIVE_UK_REGION_IDS in uk-planned-gate.mjs, and this gate's
 * assertions all land in Mark's flip commit. See docs/greater-manchester-d1/jim-handoff.md
...
assert(live?.ok === true, "assertCityLive(greater-manchester) must succeed — status is live");
...
assert(
  isMultiCity("greater-manchester") === true,
  "greater-manchester must be in MULTI_CITY_IDS"
);
...
console.log(
  `greater-manchester-dogfood-gate: ok (live, in MULTI_CITY_IDS, dispatch switch-cases wired, D1 pack, ${region.railCount} rail + ${region.metroCount} Metrolink stations, doNotGroup at Altrincham/Eccles/Manchester Airport/Rochdale (UK station fill phase 2b) and Manchester Victoria and Manchester Piccadilly/Piccadilly Gardens, WDN Walsden CRS carried without adopting West Yorkshire's WAD, National Rail directions derived live from Darwin with no static line map, exact-chip routing table (exact/undirected) proven token-free with the national rail-crs-index fallback for out-of-region termini, Metrolink dispatch correctly surfaces MetrolinkFeedUnconfirmedError rather than the static label list, all ${region.metroCount} Metrolink stops flagged liveFeed: false and excluded from the picker/Near me, Perth Australia stays green)`
);
