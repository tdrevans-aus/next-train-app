...
async function run() {
  ...
  const probe = await page.evaluate(() => {
    ...
    let pinDeparture = null;
    try {
      const result = window.nextTrainPinState.resolvePinState({
        mode: "journey",
        journey: {
          preferredTrainTime: "08:00",
          defaultFrom: "00:00",
      ...
      pinDeparture = result?.pinDeparture ?? result?.heroDeparture ?? null;
    } catch (error) {
    ...
    return {
      ...
      globalClearedOverride: fromGlobal?.journeyPinDismissedDate === "" && fromGlobal?.journeyPinOverrideDate === "",
    };
  });

  if (probe.globalType !== "function") {
    throw new Error(`expected global sanitizeJourneyPinFields function, got ${probe.globalType}`);
  }
  ...
  if (!probe.navClearedOverride || !probe.globalClearedOverride) {
    throw new Error("train-navigation / global sanitizeJourneyPinFields did not clear stale override");
  }

  const crash = pageErrors.find((msg) => /sanitizeJourneyPinFields is not defined/i.test(msg));
  if (crash) {
