/**
 * D6 — Live Vancouver SkyTrain sweep. Not a CI gate.
 *   npm run sweep:vancouver
 */
...
import { marketingLabelsForStation } from "../lib/cities/vancouver/marketing-directions.js";
...
async function main() {
  ...
  if (live?.ok !== true) {
    throw new Error("assertCityLive(vancouver) must pass");
  }
  ...
  for (const station of stations) {
    ...
    if ((board.trips ?? []).some((trip) => /seabus|west coast|bus /i.test(trip.destination))) {
      anomalies.push({ station: station.name, check: "C1", detail: "board leaked non-SkyTrain" });
    }
  }

  mkdirSync(join(ROOT, "qa/reports"), { recursive: true });
  const timestamp = new Date().toISOString().replace(/[:.]/g, "-");
  const reportPath = join(ROOT, "qa/reports", `vancouver-sweep-${timestamp}.json`);
  writeFileSync(reportPath, `${JSON.stringify({ timeZone: VANCOUVER_TIME_ZONE, anomalies, results }, null, 2)}\n`);
  console.log(`vancouver-network-sweep: ${anomalies.length} anomalies → ${reportPath}`);
}
