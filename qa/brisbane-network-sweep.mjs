/**
 * D6 — Live Brisbane network sweep (every catalog station).
 * Anomaly report, not a CI gate. Hits Translink; excluded from qa/run-all.mjs.
 *
 * Usage:
 *   npm run sweep:brisbane
 *   node qa/brisbane-network-sweep.mjs --time=am-peak --day=weekday
 *   node qa/brisbane-network-sweep.mjs --station=Central
 *
 ...
 *
 * Writes qa/reports/brisbane-sweep-<timestamp>.json and prints a summary.
 */
...
import { loadGtfsStatic, activeServicesForDate } from "../lib/providers/gtfs/static-cache.js";
...
const NAME_ALIASES = {
  ...
  "brisbane domestic airport": "domestic airport",
  "brisbane international airport": "international airport",
};
...
function linesServingStation(lineMap, stationKey) {
  return (lineMap.lines ?? []).filter((line) =>
    (line.stations ?? []).some((name) => normalizeKey(name) === stationKey)
  );
}
...
function isThroughStation(published, stationKey) {
  ...
  for (const line of published.lines ?? []) {
    ...
    if (!stations.includes(stationKey)) {
      continue;
    }
    onAny = true;
    const termini = new Set((line.termini ?? []).map(normalizeKey));
    if (!termini.has(stationKey)) {
      intermediate = true;
    }
  }
...
async function main() {
  ...
  if (args.day || args.time) {
    if (!flagsMatchClock) {
      console.warn(
        `brisbane-network-sweep: flags do not match Brisbane now (${clock.weekday} ${clock.clock}, ${actualBucket}). ` +
  ...
  if (!liveGate || liveGate.ok !== true) {
    throw new Error("assertCityLive(brisbane) must pass — city is live");
  }
  ...
  console.log("brisbane-network-sweep: fetching SEQ static + rail TripUpdates…");
  ...
  const activeServices = activeServicesForDate(staticData, now, BRISBANE_TIME_ZONE);
  ...
  const reportPath = join(reportDir, `brisbane-sweep-${timestamp}.json`);
  const report = {
  ...
  console.log(`brisbane-network-sweep: ${stations.length} stations, ${anomalies.length} anomalies`);
  console.log(`  day=${dayType} time=${timeBucket} clock=${clock.clock} ${BRISBANE_TIME_ZONE}`);
