...
console.log(
  `london-se-national-rail-dogfood-gate: ok (live, isMultiCity true, dispatch switch-cases wired, D1 pack, ${groupIds.length} station groups / 10 boards resolve, London Bridge (3) and Liverpool Street (2) internal doNotGroup sub-boards enforced via includeOperators, St Pancras International/Paddington excludeOperators enforced, King's Cross LNER resolved to in (no excludeOperators) per the UK ledger 5 Sep 2026, Euston and secondary termini correctly unbuilt, no hub configured (helper degrades to no-op), directions derived live from Darwin with no static line map, catalog CRS sweep deduplicated over ${groupIds.length} distinct CRS, routing table (exact/undirected) proven token-free, Perth stays green)`
);
