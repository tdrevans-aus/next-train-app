/**
 * Cold getRegion("uk-london-tfl") must not parse WM or Ellesmere Port catalogs.
...
assert(
  !loadedAfterWm.has("uk-ellesmere-port"),
  "WM request must not parse uk-ellesmere-port"
);

console.log("uk-catalog-lazy-load: ok (London does not parse WM/EP; WM does not parse EP)");
