/**
 * D6 — Live Rotterdam RET metro sweep. Not a CI gate.
 *   npm run sweep:rotterdam
 */
...
import { marketingLabelsForStation } from "../lib/cities/rotterdam/marketing-directions.js";
...
async function main() {
  ...
  if (live?.ok !== true) {
    throw new Error("assertCityLive(rotterdam) must pass");
  }
  ...
  for (const station of stations) {
    ...
    if ((board.trips ?? []).some((trip) => /gvb|m50|m51|m52|m53|m54|tram |bus |waterbus/i.test(trip.destination))) {
      anomalies.push({ station: station.name, check: "C1", detail: "board leaked GVB/tram/bus/waterbus" });
    }
  }

  mkdirSync(join(ROOT, "qa/reports"), { recursive: true });
  const timestamp = new Date().toISOString().replace(/[:.]/g, "-");
  const reportPath = join(ROOT, "qa/reports", `rotterdam-sweep-${timestamp}.json`);
  writeFileSync(reportPath, `${JSON.stringify({ timeZone: ROTTERDAM_TIME_ZONE, anomalies, results }, null, 2)}\n`);
  console.log(`rotterdam-network-sweep: ${anomalies.length} anomalies → ${reportPath}`);
}
