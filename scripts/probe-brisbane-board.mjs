/**
 * Probe Brisbane GTFS rail board without enabling the city in the app.
 *
 * Usage:
 *   node scripts/probe-brisbane-board.mjs Central
 *   node scripts/probe-brisbane-board.mjs Roma Street
 *   node scripts/probe-brisbane-board.mjs --list
 */
...
if (!station) {
  console.error("Usage: node scripts/probe-brisbane-board.mjs <station-name>");
  console.error("       node scripts/probe-brisbane-board.mjs --list");
  process.exit(1);
}

const gate = assertCityLive("brisbane");
if (gate.ok) {
  console.error("Unexpected: brisbane is live — probe expects planned status");
  process.exit(1);
}
