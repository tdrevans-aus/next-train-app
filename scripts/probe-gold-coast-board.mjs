/**
 * Probe Gold Coast G:link board. Public SEQ GTFS + TripUpdates — no API key.
 *
 *   node scripts/probe-gold-coast-board.mjs Helensvale
 *   node scripts/probe-gold-coast-board.mjs --list
 */
...
if (!station) {
  console.error("Usage: node scripts/probe-gold-coast-board.mjs <station-name>");
  process.exit(1);
}

const gate = assertCityLive("gold-coast");
if (!gate.ok) {
  console.error("Unexpected: gold-coast is not live");
  process.exit(1);
}
