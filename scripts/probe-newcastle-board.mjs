/**
 * Probe Newcastle Light Rail board. Same TFNSW_API_KEY as Sydney. NLR feed only.
 *
 *   node scripts/probe-newcastle-board.mjs "Newcastle Interchange"
 *   node scripts/probe-newcastle-board.mjs --list
 */
...
if (!station) {
  console.error("Usage: node scripts/probe-newcastle-board.mjs <station-name>");
  process.exit(1);
}

if (!readTfnswApiKey()) {
  console.error("TFNSW_API_KEY is not set — same key as Sydney (Open Data Hub).");
  process.exit(1);
}

const gate = assertCityLive("newcastle");
if (!gate.ok) {
  console.error("Unexpected: newcastle is not live");
  process.exit(1);
}
