#!/usr/bin/env node
/**
 * Trim OVapi NL GTFS to RET metro (route_type 1, lines A–E).
 * Does not write published-network.json (official RET Metrolijnenkaart / D1).
 *
...
async function main() {
  ...
  const routes = parseCsv(zipText(smallFiles, "routes.txt")).filter(
    (row) =>
      ...
      KEEP_SHORTS.has(String(row.route_short_name || "").trim().toUpperCase())
  );
